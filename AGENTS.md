...
* Fase 0.2.2.4 concluída: Embedding Workflow Integration Audit (`docs/architecture/embedding-policy-foundation.md`, `docs/Lina-0.2.x-Roadmap.md`, `docs/roadmap.md`, `CHANGELOG.md`, `AGENTS.md`). Auditoria arquitetural rigorosa de todos os fluxos de geração de embeddings após a introdução da Policy Foundation (0.2.2.1), Status Transparency (0.2.2.2) e Confirmation Flow (0.2.2.3): verificação exaustiva de pontos de entrada na Command Palette (`gerar-embeddings-locais`), ações de diagnóstico na Sidebar (`generate`, `update`, `rebuild`), manutenção automática (`EmbeddingScheduler`), coordenação interna (`MaintenanceEngine`, `EmbeddingWorker`, `IndexWriteCoordinator`) e chamadas a providers (Ollama, Mistral, OpenRouter); confirmação estrita de ausência de caminhos de bypass, garantia de proteção fail-fast no Mobile Companion com zero chamadas de rede ou invocação de workers, e bloqueio de consumo silencioso de APIs externas. Suíte de testes e integridade do projeto 100% validadas.
