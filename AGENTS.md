...
* Fase 9N-C3A concluída: `embeddings-enabled / PARITY-ROLLBACK` e `analysis-provider / PARITY-MUTATION` + `PARITY-SAVE-COUNT` foram adjudicados a favor da candidata; a semântica canónica passa a exigir rollback integral em save falhado, persistência antes de effects, materialização atómica de provider + URL + modelo com preservação de valores customizados, e `mark-embeddings-dirty` apenas após persistência confirmada; a UI imperativa histórica ainda divergia nessa altura e C3 mantinha-se bloqueada até C3B.
* Fase 9N-C3B concluída: a semântica canónica de save/effects foi implementada na UI imperativa e na composição candidata; `embeddings-enabled` passou a fazer rollback integral em save falhado sem effects posteriores e `analysis-provider`/`embeddings-provider` passaram a materializar provider + URL + modelo como uma mutation lógica atómica, preservando valores customizados, usando defaults apenas quando apropriado, persistindo numa única escrita e executando effects apenas depois do save confirmado; `mark-embeddings-dirty` agora depende de persistência bem-sucedida; a composição continua detached, os três findings C3 ficam resolvidos e a próxima reexecução formal de C3 é necessária antes de C4.
* Fase 9N-C3C concluída: o rollback síncrono foi reconciliado na UI imperativa, com revisão por campo para impedir que uma falha tardia reverta uma mutation posterior já confirmada; `check-sync-on-startup` e `device-name` ficaram resolvidos, a família síncrona equivalente ficou alinhada, `embeddings-model / PARITY-EFFECT-ORDER` e `binary-preference / PARITY-EFFECT-ORDER` ficaram resolvidos, a ordem canónica passou a ser `mutation → save → effect`, save failure implica rollback e nenhum effect, effect failure pós-save não implica rollback nem novo save, credenciais permanecem apenas no limite síncrono C3 já validado, a candidata continua detached, o baseline continua 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, a próxima etapa é reexecutar formalmente a C3 e a C4 continua bloqueada.
* Fase 9N-C3D concluída: o finding `C3-LATE-ROLLBACK-HISTORICAL-CALLBACKS` foi resolvido nos três callbacks históricos C3B; `embeddings-enabled` passou a usar revisão lógica para impedir que rollback tardio apague mutation posterior confirmada, `analysis-provider` e `embeddings-provider` passaram a usar revisão transacional para o trio provider + URL + modelo, rollback simples continua a funcionar, rollback tardio já não pode destruir estado confirmado de mutation posterior, não existe nova fila nem save compensatório, provider mutation mantém um save lógico, effects continuam apenas após persistência confirmada, `mark-embeddings-dirty` ocorre apenas para mutations confirmadas, effect failure pós-save não causa rollback nem novo save, regressões C3C permanecem resolvidas, a candidata continua detached, o baseline continua 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, a próxima etapa é reexecutar formalmente a C3 e a C4 continua bloqueada até essa reexecução ficar verde.
* Fase 9N-C4 BLOQUEADA: a cobertura de lifecycle assíncrono comparou callback real imperativo e lifecycle bindings da candidata, confirmou que credential Save/Replace imperativo termina com sucesso/pending prematuros sem aguardar persistência completa nem manter pending equivalente, connection test imperativo não possui busy/token/invalidação de completion tardia equivalente, falta cleanup/dispose async equivalente à candidata na UI imperativa com stale callback neutralization, descobrindo divergências materiais; pontos binários confirmam mutualização, legacy-manifest block, remove confirmation e ausência de check extra, todas as regras já corretas permanecem intactas, confirmações continuam canónicas, secrets continuam protegidos, a candidata continua detached, o baseline continua 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, a próxima subfase é `9N-C4A` para adicionar lifecycle por domínio na UI imperativa com pending/token/invalidation, e a C5 continua bloqueada até nova reexecução da C4 ficar verde.
* Fase 9N-C4A concluída: o lifecycle assíncrono imperativo por domínio foi implementado em `LinaSettingTab` sem ativar a composição candidata, com ownership explícito, pending, token/generation, duplicate-start protection, invalidation, stale-completion suppression e `dispose()` idempotente em cinco domínios (`analysis-connection`, `embeddings-connection`, `credentials-analysis`, `credentials-embeddings`, `binary`); `display()` invalida callbacks do render anterior, `hide()` invalida operações pendentes de forma segura; para connections analysis/embeddings foi implementado comportamento equivalente à candidata com uma operação por domínio, duplicate start bloqueado, pending/busy, success/failure seguros, retry após failure, completion tardia neutralizada, completion antiga não escreve feedback, rerender/cleanup invalida callbacks antigos e analysis/embeddings mantêm domínios independentes; para credentials analysis/embeddings implementou-se draft local, campo vazio, persistência aguardada antes de success, draft limpo apenas no momento canónico, failure/retry correto, duplicados no mesmo domínio bloqueados, isolamento entre analysis e embeddings, clear com confirmação e secrets fora de diagnostics/feedback/logs; para binary foi integrado no lifecycle sem alterar mutual exclusion, legacy-manifest block, remove confirmation, cancel inerte e no-extra-check, acrescentando apenas neutralização de completions tardias, pending no domínio `binary` e cleanup/invalidation sem stale status/feedback; os três findings C4 anteriores (`C4-IMPERATIVE-CREDENTIAL-LIFECYCLE`, `C4-IMPERATIVE-CONNECTION-LIFECYCLE`, `C4-IMPERATIVE-ASYNC-CLEANUP`) ficam tecnicamente resolvidos pela C4A, a candidata continua detached com 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, nenhum ficheiro técnico C4A foi commitado nesta tarefa documental, a suíte completa passa com 645 testes, typecheck passou, auditoria estática confirmou ausência de `getSettingDefinitions()`, `binary-action-feedback`, rede/I/O direto, timers, `secretStorage`, `console`, `innerHTML` e novos casts inseguros, `requestUrl` apenas pré-existente em mock, C4A NÃO conclui formalmente C4, a próxima etapa é reexecutar formalmente a Fase 9N-C4 — actions, lifecycle e cleanup, e C5 continua bloqueada até essa reexecução ficar sem findings materiais.
* Fase 9N-C4 concluída: a reexecução formal posterior à C4A aprovou a paridade material de actions, lifecycle e cleanup sem findings materiais novos; os findings históricos `C4-IMPERATIVE-CREDENTIAL-LIFECYCLE`, `C4-IMPERATIVE-CONNECTION-LIFECYCLE` e `C4-IMPERATIVE-ASYNC-CLEANUP` ficam formalmente resolvidos, com lifecycle por domínio aprovado para analysis connection, embeddings connection, credentials-analysis, credentials-embeddings e binary: pending/busy, token/generation, duplicate-start protection, stale-completion suppression, invalidation e cleanup/`dispose()` idempotente; `display()` invalida callbacks do render anterior, `hide()` invalida operações pendentes sem permitir feedback obsoleto, alteração do estado atual ou limpeza de pending de lifecycle novo; connections de analysis e embeddings mantêm domínios independentes com start, failure, retry e completion tardia validados, credentials mantêm campo vazio, draft local, persistência antes de success, retry, confirmação de clear, cancelamento inerte e isolamento de secrets, e binary preserva mutual exclusion, `legacy-manifest` block, confirmação de remove, cancelamento inerte e no-extra-check após create/update; baseline preservado em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, com `device-description`, sem placeholders ativos, `MISSING_REAL_BINDING` ou `binary-action-feedback`; a candidata continua detached, sem `getSettingDefinitions()` ativo, integração em `src/settings.ts`/`main.ts`, feature flag, tab declarativa paralela ou cutover; C3 continua concluída, a próxima fase autorizada é 9N-C5 — paridade DOM/manual e validação visual final, ainda não iniciada.
* Fase 9N-C5 BLOQUEADA: a auditoria DOM/manual e visual encontrou exclusivamente o finding `C5-BINARY-REMOVE-DESTRUCTIVE-AFFORDANCE` (`PARITY-DOM-CLASS`) no controlo `remove-binary-copy`; a UI imperativa usa `setDestructive()`, mas a action declarativa candidata não exprime affordance destrutiva equivalente, pelo que uma futura UI declarativa perderia a sinalização visual/semântica de ação destrutiva. O comportamento funcional de remove, a confirmação destrutiva e o lifecycle binário não foram reabertos; não houve finding material em headings, descriptions, controlos gerais, credentials, connections, binary status (que mantém renderer real com `aria-live="polite"`), disabled/visibility, rerender/cleanup ou segurança DOM. C3 e C4 continuam concluídas, a candidata continua detached, o baseline mantém 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, sem `getSettingDefinitions()` ativo, integração em `src/settings.ts`/`main.ts`, feature flag, tab declarativa paralela, cutover ou `binary-action-feedback`; a próxima subfase mínima é 9N-C5A — affordance destrutiva de `remove-binary-copy`, limitada à candidata e à reutilização da action/binding binária já injetada, seguida obrigatoriamente por reexecução formal de C5; C6 continua bloqueada até C5 ficar verde.
* Fase 9N-C5A concluída: o finding visual/semântico `C5-BINARY-REMOVE-DESTRUCTIVE-AFFORDANCE` (`PARITY-DOM-CLASS`) foi resolvido na candidata, ligando `remove-binary-copy` a renderer real isolado que cria exatamente um botão, aplica `setDestructive()`, respeita disabled e delega na action/binding binária já injetada; a UI imperativa e o lifecycle C4 permaneceram inalterados, assim como confirmação de remove, cancelamento inerte, mutual exclusion binária, `legacy-manifest` block, no-extra-check após create/update e ausência de `binary-action-feedback`, sem alargar o contrato global de actions. A candidata continua detached, sem `getSettingDefinitions()` ativo, integração em `src/settings.ts`/`main.ts`, feature flag, tab paralela, candidate preview ou cutover, e o baseline mantém 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta. O teste focado C5A teve `spawn EPERM` nas duas tentativas de arranque do esbuild e não é apresentado como aprovado; a suíte completa posterior passou com 51 ficheiros / 649 testes. C5A não conclui formalmente C5: a próxima etapa obrigatória é reexecutar formalmente 9N-C5, e C6 continua bloqueada até essa reexecução ficar verde.
* Fase 9N-C5 concluída: a reexecução formal pós-C5A aprovou a paridade DOM/manual e visual sem findings materiais novos, resolvendo formalmente `C5-BINARY-REMOVE-DESTRUCTIVE-AFFORDANCE` (`PARITY-DOM-CLASS`) no controlo `remove-binary-copy`; o harness observou `LinaSettingTab.display()` real e renderer candidato real com um botão, `destructive = true`, estado disabled e handler/action em ambos os lados. Foram aprovadas headings/descriptions, tipos, contagens e ordem dos controlos, credentials/connections sem regressões C3/C4, binary status com `aria-live="polite"` e diagnóstico seguro, binary actions (incluindo pending, `legacy-manifest` e affordance destrutiva), disabled/visibility, rerender/cleanup sem stale feedback ou duplicação observável e segurança DOM sem secrets, `innerHTML` inseguro, rede/I/O real, `secretStorage`, `window.confirm` ou timers introduzidos. C3 e C4 continuam concluídas; baseline preservado em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, com `device-description`, sem placeholders ativos, `MISSING_REAL_BINDING` ou `binary-action-feedback`; a candidata continua detached, sem `getSettingDefinitions()` ativo, integração em `src/settings.ts`/`main.ts`, feature flag, tab declarativa paralela, candidate preview ou cutover. A próxima fase é 9N-C6 — auditoria final pré-cutover, ainda não iniciada; esta conclusão não autoriza cutover.
* Fase 9N-C6 BLOQUEADA: a auditoria final pré-cutover aprovou a readiness funcional e arquitetural — ownership futuro, runtime adapters, bindings, renderers, lifecycle, cadeia `definition/action → composition → lifecycle controller.requestUpdate() → scheduleUpdate injetado → host update()`, credentials, connections, binary, compatibilidade de persistência, startup, API, segurança, rollback e neutralidade mobile — com baseline 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta e C3/C4/C5 concluídas; confirmou mesmo envelope de settings, `deviceSettingsById`, campos e boundaries de credentials, sem schema novo ou migração, rollback possível por revert do commit atómico, `minAppVersion` 1.13.0 compatível com `getSettingDefinitions()`/`update()` e sem dependência candidata desktop-only. O cutover permanece não autorizado exclusivamente pelo finding `C6-LINT-CANDIDATE-RUNTIME`: `npm run lint:obsidian` e `npm run lint:obsidian:strict` falham com baseline de 9 erros / 14 avisos, incluindo cinco erros bloqueantes na candidata/lifecycle — assertions desnecessárias em `declarativeSettingsCandidateComposition.ts` (linhas 218 e 231) e métodos passados sem binding em `declarativeSettingsLifecycleController.ts` (linhas 154, 186 e 233). Quatro erros em `src/settings.ts` ficam fora da correção imediata por pertencerem ao caminho imperativo ativo, e o warning `prefer-setting-definitions` é esperado enquanto esse caminho existir. A candidata continua detached, sem `getSettingDefinitions()` ativo, integração candidata em `src/settings.ts`/`main.ts`, feature flag, tab declarativa paralela, candidate preview, cutover ou `binary-action-feedback`; a próxima subfase estrita é 9N-C6A — reconciliação de lint da candidata/lifecycle, limitada aos cinco erros nos dois módulos candidatos, seguida por ambos os linters e reexecução formal de C6, sem avançar diretamente para cutover.
* Fase 9N-C6A concluída: o finding `C6-LINT-CANDIDATE-RUNTIME` foi tecnicamente resolvido exclusivamente em `declarativeSettingsCandidateComposition.ts` e `declarativeSettingsLifecycleController.ts`, removendo duas assertions redundantes `no-unnecessary-type-assertion` sem introduzir `as any`/`as unknown as`, enfraquecer type safety, alterar comportamento, bindings, definitions ou runtime, e corrigindo três findings `unbound-method` por wrapper local `reportCleanupError` que chama o callback através de `options` e preserva o contexto. Pending, tokens, generations, stale-completion suppression, cleanup, `dispose()` e `requestUpdate()` permaneceram inalterados. Os módulos C6A ficam com 0 erros de lint; o lint global ainda mantém 4 erros / 14 avisos no caminho imperativo/legado, fora do escopo C6A, pelo que não é apresentado como limpo. O baseline mantém 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, C3/C4/C5 continuam concluídas e a candidata continua detached, sem `getSettingDefinitions()` ativo, feature flag, tab paralela, candidate preview, cutover ou `binary-action-feedback`. A C6A não conclui formalmente C6: a próxima etapa obrigatória é reexecutar formalmente a auditoria final pré-cutover 9N-C6, e o cutover continua não autorizado.
* Fase 9N-C6 concluída: a reexecução formal pós-C6A aprovou a auditoria final pré-cutover com "9N-C6 CONCLUÍDA — CUTOVER AUTORIZADO"; o finding `C6-LINT-CANDIDATE-RUNTIME` foi formalmente resolvido, a composição candidata e o lifecycle controller possuem 0 erros de lint, e os 4 erros globais remanescentes pertencem exclusivamente ao caminho imperativo activo a remover/substituir no cutover; o warning `prefer-setting-definitions` em `src/settings.ts` é esperado até à remoção do renderer imperativo. Readiness matrix auditada e totalmente READY (estrutura, lint candidata/lifecycle, lint imperativo eliminável no cutover, ownership, lifecycle, update path, integração, persistência/schema, startup, API compatibility minAppVersion 1.13.0, mobile, security, I/O boundaries, dead-code plan, test strategy, rollback atómico por revert de commit sem schema/migration rollback); baseline preservado em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, com `device-description`, `remove-binary-copy` destrutivo, sem placeholders, `MISSING_REAL_BINDING` ou `binary-action-feedback`; ownership pós-cutover aprovado para `LinaSettingTab` como único owner de uma composition por instância com cadeia `definition/action → composition → controller.requestUpdate() → scheduleUpdate → update()`, sem lifecycle imperativo paralelo; 0 schema migrations, sem secrets expostos, binary safeguards intactas (mutual exclusion, legacy-manifest block, remove confirmation, cancel inerte, `setDestructive()`, no-extra-check); validação técnica confirmada (typecheck passou, bateria C2–C5 + candidata 73 testes, settings 207 testes, suíte completa 649 testes em 51 ficheiros, git diff --check passou); a candidata continua detached até à execução da próxima fase 9N-D — Cutover controlado para settings declarativas, ainda não executada.
* Fase 9N-D concluída: o cutover controlado para definições declarativas foi executado e a UI declarativa é agora o único caminho activo de produção em `LinaSettingTab` (cadeia `LinaSettingTab` → composition por instância → `getSettingDefinitions()` / `getControlValue()` / `setControlValue()` / `update()`). O caminho de produção imperativo foi removido/substituído (`display()` e `renderSettingsContent()` imperativos, lifecycle imperativo e `binaryOperationRunning` removidos). Baseline mantido em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, sem `binary-action-feedback` ou segunda fonte de verdade. 0 schema migrations, mantendo settings keys, `deviceSettingsById`, providers, modelos, URLs, preferências binárias e boundaries de credenciais (sem secrets em definitions, value hooks, DOM ou logs). Lint normal passou com 0 erros e 7 avisos históricos (eliminados os 4 erros imperativos e o aviso `prefer-setting-definitions`); lint strict falha apenas pelos mesmos 7 avisos. Suíte de testes com 51 ficheiros / 631 testes (27 ficheiros / 189 testes em settings) e typecheck/build/diff-check verdes; a redução de 649 para 631 testes decorre da migração dos harnesses C2–C5 para a tab declarativa activa e remoção de cobertura de captura imperativa sem caminho de produção. A validação manual interativa em Obsidian encontra-se PENDENTE (NÃO EXECUTADA por ausência de vault interativo no ambiente técnico) e continua obrigatória antes de qualquer release. A próxima fase é 9N-E — Limpeza pós-cutover, lint final e validação de release.
* Fase 9N-D1 concluída: corrigida a regressão transversal de persistência nos value hooks da UI declarativa ativa após a validação manual pós-cutover 9N-D. Três causas raiz resolvidas: (1) routing por chave real (`control.key`) no host do Obsidian em `getControlValue`/`setControlValue` em vez de apenas ID estrutural; (2) remoção de snapshots stale nos renderers (`declarativeSettingRenderers.ts`) reconstruindo adapters/valores a cada render; (3) sincronização do contexto runtime/device em `src/settings.ts` após replace, operações de credenciais e rollback. Ordem canónica preservada: `mutation → save confirmado → update → getter lê estado mais recente` (em falha de save, rollback síncrono sem update). Cobertura adicionada em `tests/settings/activeSettingsValueHooks.test.ts` (38 testes focados em 5 ficheiros, 196 testes de settings em 28 ficheiros, 638 testes no total em 52 ficheiros). Baseline mantido em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta. 0 schema migrations, sem reintrodução de renderers imperativos, sem alteração de labels/layout/copy e sem exposição de secretos. Lint normal com 0 erros / 7 avisos históricos; typecheck, build e diff-check verdes (`spawn EPERM` observado no sandbox mas build concluído com sucesso fora do sandbox). Validação manual interativa em Obsidian permanece PENDENTE (NÃO EXECUTADA no ambiente técnico). Próxima fase continua a ser 9N-E (ainda não iniciada), release não autorizada.
* Fase 9N-D2 concluída: corrigida a regressão específica de persistência nas transições de provider de análise e embeddings em que o model e a Base URL do provider anterior eram mantidos. Causa raiz resolvida: centralizada a matriz de defaults em `src/ai/providerDefaults.ts` (cobrindo Ollama, Mistral, OpenRouter, OpenAI, Gemini, Anthropic e Custom), eliminando a matriz duplicada e parcial em `pureLocalSettingAdapters.ts` que preservava valores antigos por falta de entradas de referência. Semântica canónica de defaults preservada: valor vazio ou igual a um default conhecido é substituído pelo novo default do provider; valores genuinamente customizados são preservados; model e Base URL são classificados independentemente. Atomicidade mantida via `setLocalProviderValues()` numa única mutation lógica (provider + model + Base URL → save → effects → update/rerender) com rollback síncrono dos três campos em falha de save e proteção contra late rollback. `setProvider()` passa a devolver sucesso confirmado e o renderer só solicita update após confirmação. Cobertura adicionada em `tests/settings/activeSettingsProviderTransitions.test.ts` (10 novos testes, 68 em bateria focada combinada, 206 em settings em 29 ficheiros, 648 testes no total em 53 ficheiros). Baseline mantido em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta. 0 schema migrations, sem alteração de UI/UX ou boundaries de credenciais. Lint normal passou com 0 erros / 7 avisos históricos; typecheck, build e diff-check verdes (com tratamento do `spawn EPERM` ambiental). Validação manual interativa em Obsidian permanece PENDENTE (NÃO EXECUTADA no ambiente técnico). Próxima fase continua a ser 9N-E (ainda não iniciada), release não autorizada.
* Fase 9N-D3 concluída: corrigida a apresentação visual das ações na UI declarativa ativa do separador de definições (`LinaSettingTab`). As `SettingDefinitionAction` deixaram de ser renderizadas como linhas clicáveis/links e passaram a usar um renderer comum com `addButton()`, transformando `test-analysis-connection` ("Test analysis connection"), `test-embeddings-connection` ("Test embeddings connection"), `check-binary-copy` ("Check binary copy") e `create-or-update-binary-copy` ("Create/update binary copy") em botões reais não destrutivos, enquanto `remove-binary-copy` ("Remove binary copy") preserva a affordance destrutiva via `setDestructive()`. O estado disabled/pending por domínio de lifecycle, handlers assíncronos e blocos de feedback técnico separados foram preservados integralmente sem duplicação. O grupo `support-the-project` ("Support the project") foi simplificado para apresentar um título, uma descrição explicativa e o link "Buy Me a Coffee" sem repetição. A apresentação de `binary-status` ("Estado da cópia binária de embeddings") passou a destacar no cabeçalho/valor principal a origem ativa (`Using binary copy` ou `Using standard index`), mantendo os detalhes técnicos completos numa linha secundária. Nenhuma alteração funcional foi efetuada aos providers de IA, motor de indexação, credenciais, lifecycle binário, persistência ou esquema de definições (0 schema migrations). Baseline mantido em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta. UI declarativa continua como único caminho ativo em produção. Lint normal passou com 0 erros / 7 avisos históricos (lint strict falha apenas pelos mesmos 7 avisos); typecheck e build verdes (com tratamento de `spawn EPERM` ambiental no esbuild); `git diff --check` passou; suíte de testes verde com 53 ficheiros / 650 testes (29 ficheiros / 208 testes em settings). A validação manual interativa em Obsidian permanece PENDENTE (NÃO EXECUTADA por ausência de ambiente interativo). A próxima fase é a Fase 9N-E (ainda não iniciada); release não autorizada.
* Fase 9N-D4 concluída: corrigida a regressão de visibilidade no renderer de credenciais declarativas que ocultava o input de API key e os botões Save/Replace e Clear nos blocos de Analysis e Embeddings. Causa raiz identificada e resolvida: o renderer especializado (`declarativeSettingsConnectionCredentialRenderers.ts`) possuía uma segunda guarda de visibilidade redundante sobre a já efetuada pela definition, provocando o retorno antecipado antes da criação dos controlos DOM quando a chave não existia. A visibilidade passou a ser owned exclusivamente pela definition. O estado final dos controlos de credenciais em Analysis e Embeddings volta a apresentar o password input (inicialmente vazio por defeito), o botão Save/Replace e o botão Clear. Todas as regras de segurança e lifecycle foram preservadas (draft local efémero, valor guardado nunca repopula o campo visível, secretos fora de definitions/diagnósticos/logs, Save/Clear acionados via bindings existentes com confirmação destrutiva no Clear, pending/lifecycle isolados por domínio). Ações convertidas na D3 mantêm-se renderizadas como botões e `remove-binary-copy` continua destrutivo. Baseline preservado em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta. 0 schema migrations, sem reintrodução de renderers imperativos nem alterações funcionais a providers/índice/cópias binárias. Suíte de testes verde com 53 ficheiros / 652 testes (29 ficheiros / 210 testes em settings; 6 ficheiros / 33 testes na bateria focada combinada). Lint normal passou com 0 erros / 7 avisos históricos (lint strict falha apenas pelos mesmos 7 avisos); typecheck e build verdes (passou fora do sandbox após `spawn EPERM` no esbuild); `git diff --check` passou. A validação manual interativa em Obsidian permanece PENDENTE (NÃO EXECUTADA no ambiente técnico). A próxima fase continua a ser a Fase 9N-E (ainda não iniciada); release não autorizada.
* Fase 9N-D6 concluída: oficializados apenas três providers de IA no caminho ativo do Lina (`Ollama`, `Mistral`, `OpenRouter`). Os providers `OpenAI`, `Gemini`, `Anthropic` e `Custom` foram descontinuados do suporte ativo e removidos da UI/dropdowns, mantendo-se apenas normalização/fallback interno para compatibilidade de leitura com definições legacy gravadas em `data.json`. Defaults centralizados em `src/ai/providerDefaults.ts`: Ollama para análise (`model: gemma4:e2b`, `Base URL: http://localhost:11434`) e embeddings (`model: nomic-embed-text-v2-moe`, `Base URL: http://localhost:11434`) sem API key; Mistral para análise (`model: mistral-small-latest`, `Base URL: https://api.mistral.ai/v1`) e embeddings (`model: mistral-embed`, `Base URL: https://api.mistral.ai/v1`) com credencial quando aplicável; OpenRouter para análise (`Base URL: https://openrouter.ai/api/v1`) com credencial e modelo configurável. As transições de provider mantêm-se atómicas (`provider + model + Base URL` → mutation lógica → save serializado → effects → update/rerender), substituindo defaults conhecidos pelos defaults do novo provider e preservando valores customizados. Regras de segurança de credenciais mantidas intactas (input vazio por defeito, o valor guardado nunca repopula o campo visível, Save e Clear explícitos, confirmação no Clear e secretos fora de diagnósticos/logs). Baseline mantido em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta. UI declarativa continua como único caminho ativo em produção. 0 schema migrations. Suíte de testes verde com 53 ficheiros / 652 testes (29 ficheiros / 210 testes em settings; 2 ficheiros / 12 testes na bateria focada de transições de provider). Lint normal passou com 0 erros / 7 avisos históricos (lint strict falha apenas pelos mesmos 7 avisos); typecheck e build verdes; `git diff --check` passou. A validação manual interativa em Obsidian permanece PENDENTE (NÃO EXECUTADA no ambiente técnico). A próxima fase continua a ser a Fase 9N-E (ainda não iniciada); release não autorizada.
* Fase 9N-D7 concluída: corrigida a renderização do controlo de modelo dependente do provider de IA no separador de definições. Causa raiz identificada e resolvida: o renderer especializado anterior renderizava sempre um dropdown e adicionava um segundo setting manual em texto, criando dois controlo de modelo ambíguos. A correção passou a decidir dinamicamente entre `addDropdown()` (para Ollama e Mistral com catálogo) e `addText()` (para OpenRouter com modelo de texto livre e default inicial vazio) na mesma definição lógica de controlo (`analysis-model` e `embeddings-model`), eliminando duplicados. Transições em tempo real na mesma instância de Settings entre Ollama ↔ OpenRouter e Mistral ↔ OpenRouter alternam dinamicamente a UI entre dropdown e textbox sem fechar a modal. O identificador do modelo OpenRouter persiste após rerender e reabertura. Preservadas as invariantes atómicas de provider (`provider + model + Base URL` → mutation lógica → save serializado → effects → update/rerender), rollback síncrono em falha de save, proteção contra late rollback, semântica custom/default, credenciais D4, botões D3, refresh de dropdown D5, providers oficiais D6 e baseline estrutural (12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta). 0 schema migrations. Suíte de testes verde com 52 ficheiros / 674 testes (28 ficheiros / 232 testes em settings; 7 ficheiros / 93 testes na bateria focada). Lint normal passou com 0 erros / 7 avisos históricos (`npm run lint:obsidian`); build passou fora do sandbox após `spawn EPERM` na primeira tentativa; `git diff --check` passou. A validação manual interativa em Obsidian permanece PENDENTE (NÃO EXECUTADA no ambiente técnico). A próxima fase continua a ser a Fase 9N-E (ainda não iniciada); release não autorizada.
* Fases 9N-D8 a 9N-D11 concluídas (consolidação de modelos manuais/custom em Ollama, Mistral e OpenRouter):
  - 9N-D8 CONCLUÍDA: introduzido o conceito de modo `Manual/custom model...` para Ollama e Mistral, com sentinel interno de UI `__lina_custom_model__` que nunca é persistido no estado real.
  - 9N-D9 CONCLUÍDA: corrigido o local de renderização do campo de texto manual usando `setting.addText()` no `Setting` ativo (em vez de criar um setting secundário fora da definição).
  - 9N-D10 CONCLUÍDA: corrigido o estado visual stale da dropdown, mantendo `Manual/custom model...` selecionado visualmente enquanto o modelo custom persistido permanece separado.
  - 9N-D11 CONCLUÍDA: separadas as atualizações de valor ("value-only update" via `setLocal()`) do rerender estrutural ("structural rerender"), permitindo a escrita contínua no textbox sem perda de foco ou de posição do cursor (caret), mantendo o rerender apenas em transições estruturais (conhecido ↔ manual ou alteração de provider).
  - Estado final dos controlos de modelo: Ollama e Mistral usam dropdown para modelos conhecidos e dropdown (`Manual/custom model...`) + textbox ativo para modelos customizados; OpenRouter usa textbox de texto livre permanente sem dropdown. Aplica-se a Analysis AI e Embeddings.
  - O sentinel `__lina_custom_model__` é estritamente UI-only. O valor persistido em `data.json` é sempre o identificador real do modelo (ex.: `my-custom-model`) ou vazio. Ao reabrir as definições com um modelo custom guardado, o estado reabre automaticamente em `Manual/custom model...` na dropdown com o valor custom no textbox. Ao selecionar um modelo conhecido na dropdown, o textbox desaparece e o modelo conhecido é persistido.
  - Preservado o baseline estrutural de 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta. UI declarativa mantida como único caminho ativo. 0 schema migrations. Providers oficiais restritos a Ollama, Mistral e OpenRouter. Boundaries de credenciais intactas (password inputs vazios, Save/Clear explícitos, confirmação no Clear e secretos fora de diagnósticos/logs). Ações em botões e affordance destrutiva preservadas.
  - Validação técnica: suíte completa com 52 ficheiros / 701 testes verdes (28 ficheiros / 259 testes em settings; 7 ficheiros / 120 testes na bateria focada); typecheck e build verdes; lint normal com 0 erros / 7 warnings históricos; `git diff --check` sem erros.
  - Validação manual: smoke test interativo do fluxo manual/custom em Obsidian com resultado PASS (escrita contínua confirmada pelo utilizador sem perda de foco/caret). Release-readiness pode ser retomada em fase posterior (9N-E), sem declarar release pronta nesta tarefa documental.
* Fase 9N-E1 concluída: release readiness da versão 0.1.15 concluída ("9N-E1 CONCLUÍDA — RELEASE READY"), sem findings materiais pendentes e com release pronta a executar em passo posterior (release ainda não executada).
  - Arquitetura final: UI declarativa consolidada como o único caminho ativo em `LinaSettingTab` (sem segunda settings tab, feature flag, preview, renderers imperativos ou schema migrations); providers oficiais consolidados exclusivamente em `Ollama`, `Mistral` e `OpenRouter` (com compatibilidade legacy de leitura para providers descontinuados); baseline canónico confirmado em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta.
  - Workflow de modelos: para Ollama e Mistral, modelos conhecidos usam dropdown e modelos customizados usam `Manual/custom model...` com textbox de texto livre com escrita contínua sem rerender destrutivo (o sentinel `__lina_custom_model__` é estritamente UI-only e o valor real do modelo é persistido); OpenRouter usa textbox permanente de texto livre sem dropdown.
  - Credenciais e segurança: Ollama com credenciais ocultas, Mistral e OpenRouter com credenciais visíveis; password inputs vazios por defeito, botões Save/Replace e Clear com confirmação destrutiva no Clear, isolamento por domínio e segredos rigorosamente fora de definitions, snapshots, logs e diagnósticos.
  - Validação técnica: Typecheck PASS; bateria focada com 9 ficheiros / 119 testes PASS; settings com 28 ficheiros / 259 testes PASS; suíte completa com 52 ficheiros / 701 testes PASS; Build PASS; `npm run release-check` PASS; `npm run release:validate` PASS; `git diff --check` PASS. Versões sincronizadas e coerentes em `manifest.json`, `package.json`, `package-lock.json` e `versions.json` (0.1.15 com `minAppVersion` 1.13.0).
  - Lint Obsidian: lint normal (`npm run lint:obsidian`) com 0 erros / 7 avisos históricos de `obsidianmd/ui/sentence-case` (classificados como non-blocking); lint strict (`npm run lint:obsidian:strict`) falha exclusivamente pelos mesmos 7 avisos históricos.
  - Validação manual:
    - PASS: apenas 3 providers visíveis; fluxo Manual/custom Ollama (Analysis e Embeddings); fluxo Manual/custom Mistral (Analysis e Embeddings); ações D3 renderizadas como botões; grupo Support simplificado; status binário legível com origem ativa destacada.
...
* Fase D1 (Integração em Runtime) concluída: inicialização de estado de dispositivo (`getOrCreateDeviceState()`) ligada ao arranque do plugin em `main.ts` (`loadDataFromDisk()`), criando/carregando deterministicamente `.lina/devices/<deviceId>.json` com `schemaVersion: 2`, preservando roles e configurações existentes, mantendo o `deviceId` sincronizado com a camada de identidade persistente e garantindo isolamento estrito entre dispositivos sem introduzir ownership, coordenação de sincronização ou alterações de UI.
* Fase D1.1 concluída: role neutro e device name estritamente humano. Removida a atribuição automática de `"producer"` na inicialização (`createDefaultDeviceState()`); novos dispositivos arrancam com `role` unassigned (`undefined`) e sem `deviceName` inferido; preservada compatibilidade retroativa e integridade de roles existentes (`"producer"` / `"companion"`) e `deviceName` user-defined sem bump de schema (`schemaVersion: 2` mantido).
* Fase D2.1 concluída: serviço e schema do manifesto de ownership (`src/device/deviceOwnership.ts`). Criação isolada do manifesto `.lina/ownership.json` com `schemaVersion: 1`, validação estrita de tipos e UUID, persistência atómica com ficheiro temporário e rename, operações de `claimInitialOwnership` (com `epoch: 1` e proteção contra sobrescrita) e `transferOwnership` (com incremento monotónico de epoch e guarda opcional contra race conditions); workers e sincronização permanecem desacoplados nesta fase.
* Fase D2.2 concluída: Worker Ownership Gating (`src/device/ownershipGate.ts`). Implementação da abstração de gating de ownership e integração em `TextIndexWorker`, `ReconciliationWorker`, `EmbeddingWorker`, `EmbeddingScheduler`, `BinaryWorker`, `MaintenanceEngine` e `main.ts` (`rebuildTextIndexInternal`). Standby producers e companions ignoram escritas em `.lina/index/*` de forma segura (sem crash, sem corrupção e sem perda de filas); todas as operações de leitura (pesquisa simples, híbrida, semântica e diagnósticos) permanecem 100% funcionais e desimpedidas. Suíte de testes validada com 75 ficheiros e 946 testes aprovados.
* Fase D2.3 concluída: Artifact Provenance Tracking (`src/device/artifactProvenance.ts`). Rastreio imutável de proveniência de artefactos partilhados (`producerDeviceId`, `producerEpoch`, `generatedAt`) nos manifestos do índice textual (`.lina/index/manifest.json`), embeddings canónicos (`manifest.embeddings.provenance`), checkpoints parciais (`embeddings.checkpoint.meta.json`) e cópias binárias (`embeddings.binary.manifest.json`). Compatibilidade retroativa a 100%: artefactos legados sem proveniência continuam a carregar com integridade e sem rebuilds forçados. Suíte de testes validada com 77 ficheiros e 974 testes aprovados.
* Fase D2.3.1 concluída: Artifact Provenance Validation Audit (`src/device/artifactProvenanceValidation.ts`). Avaliação pura, determinística e não-bloqueante de proveniência face ao estado de ownership ativo (`valid`, `stale`, `unknown`, `future`). Artefactos desatualizados (`stale`), futuros (`future`) ou legados (`unknown`) permanecem 100% utilizáveis para pesquisa sem rebuilds automáticos, reindexação forçada, alterações ao ownership ou eliminação de ficheiros. Suíte de testes validada com 78 ficheiros e 993 testes aprovados.
* Fase D2.4.1 concluída: Internal Diagnostics Model Foundation (`src/device/deviceDiagnostics.ts`). Modelo interno e snapshot tipado, determinístico e estritamente read-only (`DeviceDiagnostics`), agregando identidade/role do dispositivo, manifesto de ownership ativo e validação de proveniência de artefactos (índice textual, embeddings canónicos, cópias binárias e checkpoints) sem efeitos colaterais de escrita, auto-claims ou dependências de UI. Suíte de testes validada com 79 ficheiros e 1001 testes aprovados.
* Fase D2.4.2 concluída: Diagnostics UI / Status Panel (`src/device/deviceDiagnosticsModal.ts`). Modal e painel de estado estritamente read-only (`DeviceDiagnosticsModal`) acionado pelo comando `"mostrar-diagnostico-dispositivo"`, apresentando dados do dispositivo, estado de ownership e badges/mensagens de proveniência de artefactos (`valid`, `stale`, `unknown`, `future`) sem qualquer controlo de mutação, transferência forçada, reindexação ou duplicação de lógica de ficheiros. Suíte de testes validada com 80 ficheiros e 1004 testes aprovados.
* Fase D2.4.4 concluída: Diagnostics Internationalization Alignment (`src/device/deviceDiagnosticsModal.ts`, `src/i18n/strings.ts`). Alinhamento integral da apresentação de diagnósticos com as regras de internacionalização do Lina: títulos, cabeçalhos de secção, rótulos e valores de dispositivo/ownership, cartões de artefactos, badges de estado, explicações dinâmicas de validação de proveniência e rodapé usam exclusivamente `UiStrings` com suporte a Português (`pt-PT`) e Inglês (`en`); título corrigido para "Diagnóstico do Dispositivo e Artefactos" / "Device and Artifact Diagnostics"; separação de domínios preservada sem dependências de i18n na camada de validação e modelo interno; modal estritamente read-only preservado com 1 único botão de fecho e zero controlos de mutação. Suíte de testes validada com 80 ficheiros e 1006 testes aprovados.
* Fase D2.5.1 concluída: Manual Ownership Transfer Service Foundation (`src/device/deviceOwnershipTransfer.ts`). Criação da camada de serviço dedicada para transferências manuais controladas de ownership entre dispositivos (`transferOwnershipToDevice`), com validação rigorosa de UUID do dispositivo alvo, existência obrigatória de manifesto, rejeição de auto-transferência e proteção por expectedEpoch; incremento monotónico de época (+1) com reason `"manual-transfer"` e persistência atómica com ficheiro temporário e rename; modelo de resultados estruturados tipados sem dependências de UI, sem auto-takeovers, sem mutações de role de dispositivo e sem efeitos secundários em workers ou índices. Suíte de testes validada com 81 ficheiros e 1015 testes aprovados.
* Fase D2.5.2 concluída: Ownership Transfer Safety & Confirmation Layer (`src/device/ownershipTransferSafety.ts`). Implementação da camada de segurança e preparação prévia para transferências de ownership: geração de preview estruturado read-only sem mutações em disco (`prepareOwnershipTransferPreview`), exigência de confirmação explícita (`confirmAndExecuteOwnershipTransfer`), proteção contra stale previews com guarda de fencing de epoch (`expectedEpoch`) e validação estrita de integridade de preview (`isOwnershipTransferPreview`); delegação atómica ao serviço de transferência sem qualquer UI, sem heartbeats/TTL e sem auto-takeover. Suíte de testes validada com 82 ficheiros e 1026 testes aprovados.
* Fase D2.5.3 concluída: Diagnostics Integration for Ownership Transfer (`src/device/deviceDiagnostics.ts`, `src/device/deviceDiagnosticsModal.ts`, `src/i18n/strings.ts`). Extensão do modelo interno de diagnóstico com a secção de transferência (`DeviceDiagnosticsTransferSection`), reportando prontidão e elegibilidade estruturada (`ready`, `already-active-producer`, `missing-ownership`, `companion-role`, `unassigned-role`) sem execução de transferências, sem geração de previews de escrita, sem mutação de ownership ou role e sem efeitos secundários; apresentação estritamente read-only em `DeviceDiagnosticsModal` com suporte multilingue (`pt-PT` / `en`) via `UiStrings` e 1 único botão de fecho. Suíte de testes validada com 82 ficheiros e 1031 testes aprovados.
* Fase D2.5.4 concluída: Manual Ownership Transfer UI (`src/device/ownershipTransferConfirmationModal.ts`, `src/device/deviceDiagnosticsModal.ts`, `main.ts`, `src/i18n/strings.ts`). Implementação do workflow de transferência manual de ownership para utilizador final: ação contextual ("Promover a produtor ativo" / "Promote to active producer") visível na modal de diagnóstico quando o dispositivo é um standby producer elegível e no comando de paleta `"transferir-ownership-dispositivo"`; modal de confirmação explícita (`OwnershipTransferConfirmationModal`) apresentando estado atual (produtor ativo e época), novo estado pretendido (dispositivo alvo e próxima época) e aviso de segurança transparente (`Role != Ownership`, zero perda de dados e zero ficheiros eliminados); execução atómica e segura via `confirmAndExecuteOwnershipTransfer` com proteção contra stale-epoch race conditions, notificações de sucesso e erro traduzidas (`pt-PT` / `en`) e atualização em tempo real de diagnósticos e gate de ownership sem qualquer auto-takeover ou mutação de roles de dispositivo. Suíte de testes validada com 83 ficheiros e 1039 testes aprovados.
* Fase D2.5.5 concluída: Ownership Transfer Audit Trail Foundation (`src/device/deviceOwnershipAudit.ts`, `src/device/ownershipTransferSafety.ts`). Implementação do registo histórico imutável e append-only de transições de ownership em `.lina/ownership-history/` (`001.json`, `002.json`, ...); modelo de eventos tipado `OwnershipAuditEvent` com schema v1, IDs únicos de evento (`crypto.randomUUID()`), timestamps ISO e motivos de transição (`initial`, `manual-transfer`, `recovery-claim`); persistência atómica com ficheiro temporário e rename sem alterar o schema de `ownership.json`; integração no fluxo de confirmação e execução de transferências apenas após sucesso confirmado na persistência de ownership; leitura tolerante a falhas (`loadOwnershipAuditHistory`) com ordenação cronológica e supressão segura de ficheiros corrompidos ou inválidos; sem auto-takeover, sem heartbeats, sem mutação de roles e sem dependências de workers ou UI. Suíte de testes validada com 84 ficheiros e 1052 testes aprovados.
* Fase D2.5.6 concluída: Ownership Recovery Diagnostics Foundation (`src/device/ownershipRecoveryDiagnostics.ts`). Camada de diagnóstico e inspeção estritamente observation-only para deteção de discrepâncias e inconsistências de estado entre o manifesto de ownership ativo (`.lina/ownership.json`) e o histórico de auditoria (`.lina/ownership-history/`): deteção tipada de estados `healthy`, `missing-manifest`, `missing-history`, `history-ahead-of-manifest`, `epoch-inconsistency` e `unknown`; snapshot diagnóstico estruturado `OwnershipRecoveryDiagnostics` com identificação do produtor atual, produtor mais recente conhecido, contagem de eventos, épocas comparadas e avisos contextuais; avaliação pura síncrona (`evaluateOwnershipRecoveryState`) e inspetor assíncrono do vault (`evaluateOwnershipRecovery`); garantias estritas de zero mutações em disco, zero recriação automática de manifestos, zero auto-claims, zero efeitos secundários em workers/índices e preservação integral dos roles de dispositivo (`Role != Ownership`). Suíte de testes validada com 85 ficheiros e 1063 testes aprovados.
* Fase D2.5.7 concluída: Ownership Recovery Diagnostics UI Integration (`src/device/deviceDiagnostics.ts`, `src/device/deviceDiagnosticsModal.ts`, `src/i18n/strings.ts`). Integração da secção de diagnóstico de recuperação e coerência de ownership na interface de diagnósticos de Lina (`DeviceDiagnosticsModal` e modelo de snapshot `DeviceDiagnostics`): apresentação estritamente observation-only com badges de estado coloridos (`healthy`, `missing-manifest`, `missing-history`, `history-ahead-of-manifest`, `epoch-inconsistency`, `unknown`), comparação lado a lado da época do manifesto com a última época registada no histórico de auditoria, produtor ativo atual e último produtor conhecido, contagem total de eventos históricos e lista de avisos contextuais de integridade; suporte integral de internacionalização (`pt-PT` / `en`) via `UiStrings`; zero botões de ação ou recuperação, zero mutações no sistema de ficheiros, zero alterações de role e zero efeitos secundários em workers de manutenção. Suíte de testes validada com 85 ficheiros e 1067 testes aprovados.
* Fase D2.5.8 concluída: Ownership Architecture Hardening & Final Audit (`tests/device/ownershipArchitectureAudit.test.ts`, `docs/architecture/producer-ownership.md`, `docs/architecture/device-roles.md`, `docs/roadmap.md`, `docs/Lina-0.2.x-Roadmap.md`, `CHANGELOG.md`). Consolidação e auditoria final da arquitetura de ownership de produtor ativo (Active Producer Ownership): validação automatizada e exaustiva do ciclo de vida completo em ambientes multi-dispositivo, isolamento estrito de responsabilidades (`Role != Ownership` — zero mutações em `.lina/devices/*.json`), proteção por epoch fencing monotónico e guarda contra race conditions em transferências concorrentes, imutabilidade append-only do histórico de auditoria (`.lina/ownership-history/`) e garantias de recuperação estritamente observation-only sem efeitos secundários de escrita ou auto-claims; formalização da Matriz Completa de Estados cobrindo Estados de Dispositivo (Active Producer, Standby Producer, Companion, Unassigned), Estados de Ownership & Recuperação (`healthy`, `missing-manifest`, `missing-history`, `history-ahead-of-manifest`, `epoch-inconsistency`, `unknown`) e Estados de Proveniência de Artefactos (`valid`, `stale`, `future`, `unknown`); fundação de ownership totalmente consolidada e aprovada para transição para a Fase 0.4.x (Companion Delta Search). Suíte de testes validada com 86 ficheiros e 1087 testes aprovados.
* Fase 0.4.x concluída: Companion Delta Search Foundation (`src/companion/companionCapability.ts`, `src/companion/companionConsumptionState.ts`, `src/companion/index.ts`, `tests/companion/companionCapability.test.ts`, `tests/companion/companionConsumptionState.test.ts`, `docs/architecture/companion-delta-search-foundation.md`). Criação da fundação arquitetural para dispositivos Companion consumirem conhecimento sincronizado sem rebuild local do índice: modelo de deteção de capacidades do Companion (`CompanionCapability`) com desativação estrita de geração de embeddings e escritas no índice partilhado; modelo de estado de consumo de artefactos (`CompanionArtifactConsumptionState`) estritamente read-only descrevendo última época conhecida do produtor, versão do índice, estado de embeddings e validação não-bloqueante de proveniência (`valid`, `stale`, `unknown`, `future`); garantias estritas de zero mutações pelo Companion e total independência de sincronização face a ferramentas externas (Syncthing, Obsidian Sync, sistema de ficheiros). Suíte de testes validada com 88 ficheiros e 1106 testes aprovados.
* Fase 0.4.1 concluída: Companion Delta Search Read-Only Query Layer (`src/companion/companionSearch.ts`, `src/companion/index.ts`, `tests/companion/companionSearch.test.ts`, `docs/architecture/companion-delta-search-foundation.md`). Implementação da camada de consumo e execução de pesquisas para dispositivos Companion: delegação de consultas textuais e semânticas (`executeCompanionTextSearch`, `executeCompanionSemanticSearch`, `executeCompanionSearch`) com seleção automática de modo, resiliência total face a ausência de embeddings (fallback textual automático) e usabilidade não-bloqueante com proveniência stale, legacy ou futura; garantias estritas de zero escritas no disco, zero mutações de ownership e zero invocação de workers de manutenção. Suíte de testes validada com 89 ficheiros e 1119 testes aprovados.
* Fase 0.4.2.1 concluída: Companion Search Diagnostics & Capability Exposure (`src/device/deviceDiagnostics.ts`, `src/device/deviceDiagnosticsModal.ts`, `src/i18n/strings.ts`, `tests/device/deviceDiagnosticsCompanion.test.ts`, `docs/architecture/companion-delta-search-foundation.md`). Extensão do modelo de diagnósticos (`DeviceDiagnostics`) e da interface modal (`DeviceDiagnosticsModal`) com a secção de diagnóstico de pesquisa Companion: exposição tipada e estritamente observation-only da disponibilidade de pesquisa, modos de operação (`full`, `text-only`, `degraded`, `unavailable`), prontidão de artefactos (índice textual e embeddings) e notas contextuais; internacionalização completa em `pt-PT` e `en`; garantias de zero mutações no disco, zero alterações de ownership e zero arranque de workers. Suíte de testes validada com 90 ficheiros e 1129 testes aprovados.
* Fase 0.4.3 concluída: Search Architecture Audit & Companion Integration Planning (`docs/architecture/companion-delta-search-foundation.md`, `docs/roadmap.md`). Auditoria abrangente do ciclo de vida de pesquisa (UI, comandos, motores textuais, semânticos e híbridos, e camada de consumo Companion). Definição da arquitetura-alvo: Um Único Motor de Pesquisa com comportamento adaptável às capacidades do dispositivo (`Role != Ownership`), eliminando duplicação de motores ou interfaces divergentes.
* Fase 0.4.4 concluída: Local Delta Search Foundation (`src/companion/companionDeltaSearch.ts`, `src/companion/index.ts`, `tests/companion/companionDeltaSearch.test.ts`, `docs/architecture/companion-delta-search-foundation.md`). Implementação da camada de pesquisa delta local para dispositivos Companion: deteção puramente em memória de notas recém-criadas, modificadas ou eliminadas face ao índice publicado; chunking e estruturas de pesquisa efêmeras construídas sob demanda; execução de pesquisas textuais delegadas diretamente no motor `searchTextIndex` existente; fusão e desduplicação de resultados com precedência de deltas modificados sobre registos de índice obsoletos (`fuseSearchResults`, `executeCompanionSearchWithDelta`); garantias estritas de zero escritas no disco, zero mutações de ownership, zero geração de vetores e zero arranque de workers. Suíte de testes validada com 91 ficheiros e 1137 testes aprovados.
* Fase 0.2.2.1 concluída: Embedding Policy Foundation (`src/ai/providerCapabilities.ts`, `src/maintenance/embeddingPolicyEngine.ts`, `tests/maintenance/embeddingPolicyEngine.test.ts`, `docs/architecture/embedding-policy-foundation.md`). Criação da fundação desacoplada de decisão para atualizações de embeddings: modelo puro de capacidades de providers (`EmbeddingProviderCapability`) distinguindo transporte local de custos externos por token; motor puro de decisão de políticas (`evaluateEmbeddingUpdatePolicy`) avaliando capacidade, política do utilizador (`"manual"` / `"automatic-local-only"`), role do dispositivo (`"producer"` / `"companion"`) e estado de trabalho pendente; proteção garantida contra consumo silencioso de APIs externas e desativação estrita de geração de embeddings no Companion; zero efeitos secundários no disco, rede, scheduler ou workers nesta fase. Suíte de testes validada com 92 ficheiros e 1152 testes aprovados.
* Fase 0.2.2.2 concluída: Embedding Status Transparency (`src/maintenance/embeddingStatusExplanation.ts`, `src/i18n/strings.ts`, `tests/maintenance/embeddingStatusExplanation.test.ts`, `docs/architecture/embedding-policy-foundation.md`). Criação da camada pura de explicação e apresentação humanizada de estado de embeddings (`EmbeddingStatusExplanation`, `explainEmbeddingStatus`): transformação de contagens técnicas e decisões de política em títulos, resumos e detalhes transparentes; avaliação explícita do impacto em pesquisa semântica (`complete`, `partial`, `unavailable`), sinalização inequívoca de risco de custos de créditos de API (`mayConsumeCredits: boolean`), explicação de restrições do modo Companion e recomendação de ações (`update`, `review-policy`, `none`); suporte integral multilingue em Português europeu (`pt-PT`) e Inglês (`en`) em `UiStrings`; zero efeitos secundários de escrita, chamadas de rede ou invocação de workers. Suíte de testes validada com 93 ficheiros e 1163 testes aprovados.
* Fase 0.2.2.3 concluída: Embedding Update Confirmation Flow (`src/maintenance/embeddingUpdateConfirmation.ts`, `src/maintenance/embeddingUpdateConfirmationModal.ts`, `main.ts`, `src/search/linaSearchView.ts`, `src/i18n/strings.ts`, `tests/maintenance/embeddingUpdateConfirmation.test.ts`, `docs/architecture/embedding-policy-foundation.md`). Implementação do fluxo de confirmação explícita de utilizador antes da geração de embeddings: modelo puro de preparação de confirmação (`prepareEmbeddingUpdateConfirmation`, `EmbeddingUpdateConfirmationRequest`) consolidando capacidades de provider, decisão da policy engine, escopo de trabalho e avisos localizados de custos; modal dedicada de confirmação (`EmbeddingUpdateConfirmationModal`) apresentando identidade de provider/modelo, contagens de embeddings a gerar/substituir, aviso proeminente de consumo de créditos de API externa e impacto na pesquisa semântica; integração de gating de confirmação nos pontos de entrada manuais em `main.ts` (`gerar-embeddings-locais`) e `src/search/linaSearchView.ts` (ações de diagnóstico da sidebar `generate`, `update`, `rebuild`) via `confirmAndRequestEmbeddingGeneration`; garantia estrita contra consumo silencioso de APIs externas, defesa fail-fast no Companion (`PRODUCER_OPERATION_UNAVAILABLE_MESSAGE`) e delegação unificada ao pipeline de geração existente sem duplicação de workers ou schedulers. Suíte de testes validada com 94 ficheiros e 1172 testes aprovados.
* Fase 0.2.2.4 concluída: Embedding Workflow Integration Audit & Embedding Update Settings (`src/maintenance/embeddingUpdateSettings.ts`, `src/settings.ts`, `src/settings/declarativeGlobalSettings.ts`, `src/settings/pureGlobalSettingDefinitions.ts`, `src/settings/pureGlobalSettingsComposition.ts`, `src/settings/pureDeclarativeSettingsBlueprint.ts`, `src/settings/declarativeSettingsCandidateComposition.ts`, `src/settings/settingsRuntimeAdapters.ts`, `main.ts`, `src/i18n/strings.ts`, `tests/settings/embeddingUpdateSettings.test.ts`, `docs/architecture/embedding-policy-foundation.md`). Auditoria arquitetural de fluxos de trabalho e implementação da camada de configuração de preferências do utilizador para atualizações de embeddings: modelo puro `EmbeddingUpdateSettings` com `embeddingUpdateMode` (`"manual"` vs `"automatic-local-only"`), predefinido de forma conservadora para `"manual"`; integração limpa no schema de settings (`LinaSettings`, `DEFAULT_SETTINGS`), blueprint declarativo puro, renderers de dropdown e adapters de runtime com garantia de zero efeitos secundários de execução a partir da camada de settings; resolução de políticas em `main.ts` diretamente ligada à preferência configurada preservando a proteção fail-fast no Companion e avisos explícitos de custos em providers externos; suporte bilingue completo em `pt-PT` e `en`. Suíte completa validada com 95 ficheiros e 1187 testes aprovados.
* Fase 0.2.2.5 concluída: Scheduler Integration (`main.ts`, `tests/maintenance/automaticEmbeddingRuntimeDispatch.test.ts`, `docs/architecture/embedding-policy-foundation.md`, `docs/roadmap.md`, `docs/Lina-0.2.x-Roadmap.md`, `CHANGELOG.md`, `AGENTS.md`). Integração completa do scheduler de background com a policy engine centralizada: ligação direta de `EmbeddingScheduler.canDispatchAutomatically` a `evaluateEmbeddingUpdatePolicy`, considerando a preferência do utilizador (`embeddingUpdateMode`), capacidades do provider (`isLocal`, `hasExternalCost`) e role operacional do dispositivo (`"producer"` vs `"companion"`); autorização de geração automática em background estritamente para providers locais (Ollama) em Desktop Producer sob a política `"automatic-local-only"`; bloqueio total de dispatch automático em background para providers externos (Mistral, OpenRouter) e sob o modo `"manual"`, prevenindo sem exceção consumo silencioso de créditos de API; desativação do scheduler no Mobile Companion; testes dedicados de runtime dispatch validados. Suíte completa aprovada com 95 ficheiros e 1191 testes.
* Fase 0.2.2.6 concluída: Backoff Protection (`src/maintenance/embeddingBackoffPolicy.ts`, `src/maintenance/embeddingScheduler.ts`, `tests/maintenance/embeddingBackoffPolicy.test.ts`, `tests/maintenance/embeddingScheduler.test.ts`, `tests/maintenance/automaticEmbeddingRuntimeDispatch.test.ts`, `docs/architecture/embedding-policy-foundation.md`, `docs/roadmap.md`, `docs/Lina-0.2.x-Roadmap.md`, `CHANGELOG.md`, `AGENTS.md`). Implementação da camada pura de resiliência e backoff exponencial para manutenção automática de embeddings: modelo determinístico `EmbeddingBackoffPolicy` com progressão exponencial de cooldown (1m, 2m, 4m, 8m, 15m cap) após falhas em background; integração de rastreio de falhas e bloqueio por cooldown em `EmbeddingScheduler` sem perda do estado dirty de trabalho; reset imediato de backoff após geração bem-sucedida, preempção manual (`preemptForManual()`) ou limpeza de estado; garantias estritas de zero tentativas automáticas para providers externos e zero sobrecarga no Mobile Companion. Suíte completa aprovada com 96 ficheiros e 1202 testes.
...
* O estado canónico do índice textual deriva exclusivamente dos artefactos persistidos válidos (`manifest.json`, `notes.json`, `chunks.jsonl`) através de `readTextIndexStatus()`, alinhando startup, barra lateral e pesquisa. A decisão de usabilidade distingue `missing`, `invalid`, `ready` e `stale`; a ausência de metadados de proveniência implica origem `unknown`, o que não impede a sua utilização normal. Índices válidos produzidos noutros dispositivos e sincronizados (e.g. Syncthing) são consumidos sem exigir criação ou reconstrução local.
* Operações de renomeação e movimentação de notas (`rename`/`move`) são tratadas como uma substituição lógica atómica: todos os registos candidatos antigos e novos (`path` e `oldPath`) são removidos do snapshot, a elegibilidade do destino é validada contra as regras de exclusão (pastas, termos de caminho e termos de conteúdo), e novas entradas e chunks só são publicados se o destino for elegível. Se o destino estiver excluído, a publicação antiga é eliminada. Chunks antigos tornam-se obsoletos e o estado de trabalho de embeddings fica dirty sem geração automática de vetores. A pesquisa textual, semântica e sidebar filtram ativamente resultados sem `TFile` válido no Vault.
...
- Próxima fase: **9N-C3 — Reexecutar formalmente controls, persistência e effects**.
...
- A 9N-C1 concluiu a instrumentação do harness sem integração ativa; a próxima fase é a reexecução formal de C3 (controls, persistência e effects) depois de C3B.
