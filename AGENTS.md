...
* Fase 9N-C3 BLOQUEADA após reexecução pós-C3C: os findings C3C continuam resolvidos e o fluxo normal está correto, mas foi descoberto um novo finding `C3-LATE-ROLLBACK-HISTORICAL-CALLBACKS` com tipos `PARITY-ROLLBACK`, `PARITY-PRESERVATION`, `PARITY-SAVE-ORDER`; afeta `embeddings-enabled`, `analysis-provider` e `embeddings-provider` em cenários de concorrência tardia (mutation A pendente → mutation B confirmada → A falha), onde os callbacks históricos ainda usam `persistWithRollback` sem proteção por revisão/transação e podem reverter estado posterior confirmado; a candidata preserva o estado posterior, revelando divergência material; a próxima subfase é `9N-C3D` para proteger rollback tardio nos três callbacks históricos, mantendo um save lógico e effects pós-persistência, e a C4 continua bloqueada até nova reexecução da C3 ficar verde.
