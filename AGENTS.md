...
* Fase 0.2.3.2 concluída: Settings & UX Reorganization Implementation (`src/i18n/strings.ts`, `src/settings/pureDeclarativeSettingsBlueprint.ts`, `src/settings/declarativeSettingRenderers.ts`, `src/settings/declarativeSettingsCandidateComposition.ts`, `src/settings.ts`, `tests/settings/*`, `docs/architecture/settings-information-architecture.md`, `README.md`, `docs/roadmap.md`, `docs/Lina-0.2.x-Roadmap.md`, `CHANGELOG.md`, `AGENTS.md`). Reorganização completa da arquitetura de informação das definições em 3 níveis progressivos segundo o princípio "Mostrar aos utilizadores o que precisam para operar o Lina; esconder complexidade técnica até ser necessária":
  - (1) Definições Básicas: essenciais do dia-a-dia, cabeçalho do Lina, informação e nome do dispositivo (identificação clara de Desktop Producer vs Mobile Companion), configuração de IA (fornecedor de análise, modelo, credenciais e teste de ligação — a seleção do modelo permanece nas Definições Básicas), pesquisa semântica (ativação, fornecedor, modelo, modo de atualização de embeddings, credenciais e teste), pasta Inbox, auto-atualização do índice, exclusões simples, propriedades YAML e idioma de interface;
  - (2) Definições Avançadas: endpoints customizados (Base URLs), timeouts, tamanho de lote (trechos de notas por execução), língua dos embeddings, reindexação no arranque, número máximo de notas da Inbox, pesos da pesquisa híbrida, propriedades YAML detalhadas, limite de tags e filtros avançados de caminho e conteúdo;
