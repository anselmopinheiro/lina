...
* Fase 9N-C3 BLOQUEADA após a reexecução formal: os findings C3B continuam resolvidos, mas surgiram `PARITY-ROLLBACK` em `check-sync-on-startup` e `device-name` (e na família equivalente de controls síncronos persistidos) e `PARITY-EFFECT-ORDER` em `embeddings-model` e `binary-preference`; a semântica canónica mantém rollback integral em save failure e `mutation → save → success → effect`, a candidata continua detached, o baseline continua em 12 grupos / 47 IDs estruturais / 47 definitions reais / 0 bindings em falta, a C3C é a próxima subfase permitida e a C4 continua bloqueada.
...
- Próxima fase: **9N-C3C — Reconciliar rollback e ordem dos effects nos controlos síncronos remanescentes**.
