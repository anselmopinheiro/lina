...
* Fase 0.2.2 concluída: Embedding Update Lifecycle (`src/ai/providerCapabilities.ts`, `src/maintenance/embeddingPolicyEngine.ts`, `src/maintenance/embeddingStatusExplanation.ts`, `src/maintenance/embeddingUpdateConfirmation.ts`, `src/maintenance/embeddingUpdateConfirmationModal.ts`, `src/maintenance/embeddingUpdateSettings.ts`, `src/maintenance/embeddingScheduler.ts`, `src/maintenance/embeddingBackoffPolicy.ts`, `docs/architecture/embedding-policy-foundation.md`). Conclusão integral do ciclo de vida de atualização de embeddings vetoriais (Fases 0.2.2.1 a 0.2.2.6):
  - *Garantias Arquiteturais Consolidadas:* (1) Zero consumo silencioso de créditos de APIs externas; (2) Proteção estrita do Mobile Companion em modo puramente read-only (zero geração de embeddings e zero arranque de schedulers); (3) Gating por confirmação explícita do utilizador para providers externos ou modo manual; (4) Execução unificada delegada exclusivamente ao `MaintenanceEngine` / `EmbeddingWorker` existente sem duplicação de pipelines; (5) Resiliência e proteção contra falhas em background via backoff exponencial (1m a 15m) com reset imediato em ações manuais do utilizador.
* Fase 0.2.2.7 concluída: Companion Audit (`src/companion/companionCapability.ts`, `src/companion/companionConsumptionState.ts`, `src/companion/companionSearch.ts`, `src/companion/companionDeltaSearch.ts`, `src/device/ownershipGate.ts`, `src/maintenance/maintenanceEngine.ts`, `docs/roadmap.md`, `docs/Lina-0.2.x-Roadmap.md`, `CHANGELOG.md`, `AGENTS.md`). Auditoria arquitetural exaustiva confirmando o isolamento e estrito cumprimento dos limites do modelo Desktop Producer + Mobile Companion após a integração do ciclo de vida de embeddings: verificação formal de zero geração de vetores no Companion, zero arranque de schedulers de background, zero mutações nos ficheiros de índice partilhado ou ownership, resiliência não-bloqueante na pesquisa com fallback textual automático, pesquisa delta local 100% efêmera em memória e diagnósticos transparentes. Próxima fase: Estabilização de Release e Polish.
* Fase Pre-0.2.3 concluída: Secret Boundary Cleanup (`src/settings.ts`, `src/device/secretStorage.ts`, `tests/security/secretBoundaryProtection.test.ts`, `tests/device/secretStorage.test.ts`, `docs/architecture/secrets-and-obsidian-storage.md`, `README.md`, `docs/roadmap.md`, `docs/Lina-0.2.x-Roadmap.md`, `CHANGELOG.md`, `AGENTS.md`). Blindagem e garantia estrita do limite de credenciais (Secret Boundary): (1) Setters legados `setLocalAnalysisApiKey` e `setLocalEmbeddingsApiKey` atualizados para persistir exclusivamente em `SecretStorage` (`LINA_SECRET_KEYS`) quando disponível, eliminando chamadas a `setLocalVal()` e limpando chaves em texto limpo pré-existentes em `deviceSettingsById` ou `activeSettings` (`data.json`); (2) Expansão de `migrateLegacyCredentials` para detetar, migrar e purgar chaves de raiz (`analysisApiKey`, `embeddingsApiKey`, `aiApiKey`, `embeddingApiKey`) e entradas entre múltiplos dispositivos em `deviceSettingsById`; (3) Campos de credenciais em texto limpo marcados formalmente como `@deprecated` para compatibilidade retroativa sem quebras estruturais; (4) Verificação exaustiva da garantia de zero sync de segredos para dispositivos Companion. Suíte de testes validada com 97 ficheiros e 1209 testes aprovados.
* Fase 0.2.3.1 concluída: Architecture Consistency Audit (`docs/roadmap.md`, `CHANGELOG.md`, `AGENTS.md`). Auditoria arquitetural completa validando a separação rigorosa entre configuração partilhada (`data.json`), estado local por dispositivo (`.lina/devices/<deviceId>.json`), credenciais e chaves de API (`app.secretStorage`), artefactos publicados (`.lina/index/`) e memória runtime efêmera. Confirmação de zero caminhos de fuga de segredos em canais de sincronização e preservação total das invariantes do modelo Desktop Producer + Mobile Companion.
* Fase 0.2.3.2 concluída: Settings & UX Reorganization & Intent Alignment (`src/i18n/strings.ts`, `src/settings/pureDeclarativeSettingsBlueprint.ts`, `src/settings/declarativeSettingRenderers.ts`, `src/settings/declarativeSettingsCandidateComposition.ts`, `src/settings.ts`, `tests/settings/*`, `docs/architecture/settings-information-architecture.md`, `README.md`, `docs/roadmap.md`, `docs/Lina-0.2.x-Roadmap.md`, `CHANGELOG.md`, `AGENTS.md`). Reorganização e alinhamento completo da arquitetura de informação das definições em 3 níveis progressivos segundo os princípios "Configuração deve ser agrupada por intenção e funcionalidade do utilizador, não por implementação técnica" e "Mostrar aos utilizadores o que precisam para operar o Lina; esconder complexidade técnica até ser necessária":
  - (1) Definições Básicas (Fluxo Completo de Configuração de Providers): os utilizadores podem configurar integralmente um provider de Análise IA (provider, modelo, URL base, credencial e teste de ligação com feedback instantâneo) e Pesquisa Semântica (toggle de ativação, provider, modelo, URL base, credencial, modo de atualização e teste de ligação) sem nunca necessitarem de abrir as Definições Avançadas; inclui cabeçalho com versão e build, identificação destacada do dispositivo com badge visual de alta visibilidade (`🟢 Desktop Producer` / `🔵 Mobile Companion`) e explicação clara do que o dispositivo pode fazer, nome do dispositivo, pasta Inbox, auto-atualização do índice, exclusões simples, propriedades YAML e idioma de interface;
  - (2) Definições Avançadas (Ajuste Técnico e Afinação): reservadas estritamente para parâmetros técnicos e afinação de performance (timeouts de análise e embeddings, tamanho de lote de trechos de notas, língua dos embeddings, reindexação no arranque, número máximo de notas da Inbox, pesos da pesquisa híbrida, propriedades YAML detalhadas, limite de tags e filtros avançados de caminho e conteúdo);
  - (3) Manutenção e Diagnóstico: verificação de sincronização no arranque, registo de depuração e gestão da cache de pesquisa rápida / aceleração (verificação, criação/atualização e remoção com confirmação destrutiva);
