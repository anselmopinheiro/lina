...
describe("pure local settings model", () => {
  ...
  it("mirrors the provider order and pure metadata used by local settings", () => {
    expect(getPureLocalProviderOptions()).toEqual([
      ...
      { value: "openrouter", label: "OpenRouter" }, { value: "openai", label: "OpenAI" },
      { value: "gemini", label: "Gemini" }, { value: "anthropic", label: "Anthropic" },
    ...
    expect(getPureLocalProviderMetadata("openai")).toMatchObject({ isLocal: false, requiresApiKey: true, hasModelCatalog: false });
  ...
  it("reuses static catalogs and preserves manual-model handling", () => {
    ...
    expect(getPureLocalModelOptions("openai", "analysis")).toEqual([]);
  ...
  it("keeps visibility conditions pure and provider-only", () => {
    ...
    expect(shouldShowPureLocalManualModel("custom")).toBe(true);
