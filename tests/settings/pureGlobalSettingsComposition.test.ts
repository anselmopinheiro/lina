...
const expectedSectionKeys = [
  ["embeddingsEnabled", "embeddingUpdateMode"],
...
describe("pure global settings composition", () => {
  it("contains only the five approved sections and their controls in current visual order", () => {
    ...
    expect(getControlKeys("pt-PT")).toEqual(expect.arrayContaining([...DECLARATIVE_GLOBAL_SETTING_KEYS]));
    expect(new Set(getControlKeys("pt-PT")).size).toBe(12);
  });
  ...
  it("does not add declarative informational rows that need custom rendering", () => {
    ...
    expect(composition.flatMap((section) => section.items ?? [])).toHaveLength(12);
