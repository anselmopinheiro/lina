...
**Status:** Implemented (Phase D1)  
**Scope:** Definition of the `DeviceRole` model, separation of identity, capabilities, and roles, and persistence within device-scoped state (`.lina/devices/<deviceId>.json`).
...
└────────────────────────────────────┬─────────────────────────────────────┘
                                     ...
                                     ▼
┌──────────────────────────────────────────────────────────────────────────┐
│ 3. Device Role (Phase D1 — Implemented)                                  │
│    • Answers: "How should Lina use this installation?"                   │
│    • Operational role: "producer" | "companion"                          │
...
The `DeviceRole` type defines two initial roles:
...
* **Defaults:** Assigned by default to desktop installations.
...
* **Defaults:** Assigned by default to mobile installations.
...
2. **Backward Compatibility:** `loadDeviceState` seamlessly reads legacy `schemaVersion: 1` files and defaults omitted roles safely without data loss.
...
Phase D1 prepares the foundation for subsequent ownership and coordination phases:
