...
- **Settings Grouped by User Intent and Functionality:**
  ...
  - **Basic Settings (Complete Provider Setup Flow):**
    - Users can configure AI Analysis completely in Basic: provider selection, model selection, custom/local Base URL endpoint, API key configuration, and connection test with immediate feedback.
    - Users can configure Semantic Search completely in Basic: enable toggle, provider, model, custom/local Base URL endpoint, API key configuration, embedding update policy (`manual` vs `automatic-local-only`), and connection test with immediate feedback.
    - Preserves everyday essentials: active device role & name, inbox folder, index auto-update, exclusions, YAML toggles, and interface language.
  - **Advanced Settings (Tuning & Overrides):**
    - Reserved for tuning rather than completing setup: request timeouts, batch processing sizes (note passages per batch), startup index reindexing, maximum inbox notes, hybrid search balance weights, advanced YAML properties list, and path/content exclusion filters.
  - **Diagnostics & Maintenance:** Houses index diagnostics (sync verification on startup, debug logging) and fast search cache management (check, create/update, and remove with destructive confirmation affordance).
- **High-Visibility Producer / Companion Device Role Indicator:**
  - Prominently displays the active device role with a clear visual badge: `🟢 Desktop Producer` ("This device maintains the search index and generates embeddings") or `🔵 Mobile Companion` ("This device uses synchronized search data generated by your Desktop Producer").
  - Architectural role clarity without artificial dropdown preferences.
  - On Mobile Companion devices, misleading background generation controls (such as embedding update mode) are cleanly gated and accompanied by clear Companion mode notices ("Companion mode active — This device uses search data generated by your Desktop Producer").
- **Terminology Polish (PT-PT and EN):**
  - Updated technical jargon to user-friendly concepts: "Cópia binária" / "Binary copy" → "Cache de aceleração da pesquisa" / "Fast search cache"; "Chunks" → "Trechos de notas" / "Note passages"; "Proveniência de artefactos" → "Estado dos dados de pesquisa" / "Search data status".
- **Zero Breaking Changes & Parity Integrity:**
  - Preserved all 49 existing settings items, actions, and underlying configurations with 100% test parity, zero schema migrations, and zero breaking changes.
...
### Changed
- Updated development toolchain dependencies with esbuild 0.25.12.
