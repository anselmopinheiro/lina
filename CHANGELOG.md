...
- Prepared the main declarative settings infrastructure while keeping it disconnected from the active UI, which remains imperative via `display()`. A pre-cutover audit identified two still-imperative active controls not yet represented in the blueprint (`autoUpdateIndexOnFileChanges` and `maxSuggestedTags`), so cutover remains blocked until blueprint completion, production adapters, lifecycle validation, and parity-harness validation.
