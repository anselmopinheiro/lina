...
var PT_PT = {
  ...
  diagnosticEmbeddingDetailsUnavailable: "Estado detalhado ainda n\xE3o calculado nesta sess\xE3o.",
  ...
  settingsCredentialNotStored: "N\xE3o guardada",
  ...
  settingsCredentialClearSuccess: "Opera\xE7\xE3o de limpeza da chave API conclu\xEDda.",
  settingsCredentialOperationError: "N\xE3o foi poss\xEDvel atualizar a chave API.",
...
function normalizeProviderBaseUrlForComparison(provider, value) {
  switch (provider) {
    case "ollama":
...
function isKnownDefaultBaseUrl(value) {
  return Object.entries(PROVIDER_BASE_URL_DEFAULTS).some(([provider, defaultUrl]) => {
    return normalizeProviderBaseUrlForComparison(provider, value) === normalizeProviderBaseUrlForComparison(provider, defaultUrl);
  });
}
...
function chooseProviderDefaultModel(currentModel, provider, type) {
  ...
  const isValidForTarget = getProviderModels(provider, catalogType).some((model) => model.id === trimmedCurrent);
  const isKnownForAnotherProvider = ["ollama", "mistral", "openrouter"].some((candidate) => getProviderModels(candidate, catalogType).some((model) => model.id === trimmedCurrent));
...
function sanitizeApiMessage3(message) {
  const singleLine = message.replace(/\s+/g, " ").trim();
  const redacted = singleLine.replace(/Bearer\s+[A-Za-z0-9._~+/=-]+/gi, "Bearer [redacted]").replace(/api[_ -]?key\s*[:=]\s*[A-Za-z0-9._~+/=-]+/gi, "api key [redacted]");
  return redacted.length > 220 ? `${redacted.slice(0, 217)}...` : redacted;
}
function extractSafeApiMessage3(value) {
  if (typeof value === "string") return sanitizeApiMessage3(value);
  if (Array.isArray(value)) {
    for (const item2 of value) {
      const message = extractSafeApiMessage3(item2);
      if (message) return message;
    }
    return void 0;
  }
  ...
  for (const key of ["message", "detail", "error", "code"]) {
    const nested = value[key];
    if (typeof nested === "string") return sanitizeApiMessage3(nested);
    const message = extractSafeApiMessage3(nested);
    if (message) return message;
  }
  return void 0;
}
function describeHttpFailure(status) {
  switch (status) {
    case 400:
      return "O pedido de embeddings para OpenRouter \xE9 inv\xE1lido. Verifica o modelo e os inputs.";
    case 401:
      return "Chave API OpenRouter inv\xE1lida ou em falta.";
    case 402:
      return "A conta OpenRouter n\xE3o tem cr\xE9ditos suficientes para embeddings.";
    case 404:
      return "O modelo OpenRouter n\xE3o existe ou n\xE3o suporta embeddings.";
    case 429:
      return "O limite de pedidos OpenRouter foi atingido. Tenta novamente mais tarde.";
    case 529:
      return "O provider de embeddings OpenRouter est\xE1 temporariamente sobrecarregado.";
    default:
      return `OpenRouter respondeu com status ${status}.`;
  }
}
function classifyHttpFailure(status) {
  switch (status) {
    case 400:
      return { errorCategory: "configuration", errorScope: "operation", fatal: true };
    case 401:
      return { errorCategory: "authentication", errorScope: "operation", fatal: true };
    case 402:
      return { errorCategory: "billing", errorScope: "operation", fatal: true };
    case 404:
      return { errorCategory: "model-not-found", errorScope: "operation", fatal: true };
    case 429:
      return { errorCategory: "rate-limit", errorScope: "operation", fatal: true };
    case 529:
...
async function generateOpenRouterEmbeddings(baseUrl, apiKey, model, inputs, timeoutMs = 6e4) {
  ...
  if (!apiKey.trim()) {
    return operationError("configuration", "Chave API OpenRouter em falta. Define uma chave local nas defini\xE7\xF5es do Lina.", {
      provider: "openrouter",
      endpoint,
      requestCount: 0
    });
  ...
  if (inputs.length === 0) {
    return operationError("configuration", "N\xE3o existem inputs para gerar embeddings com OpenRouter.", {
      provider: "openrouter",
      endpoint,
      requestCount: 0
    });
  ...
  try {
    ...
    const request = (async () => {
      ...
      if (response.status !== 200) {
        return {
          ...
          endpoint,
          status: response.status,
          apiMessage: extractSafeApiMessage3(response.json),
          requestCount: 1,
      ...
      const data = response.json;
      if (!Array.isArray(data.data) || data.data.length !== inputs.length) {
        return operationError("invalid-response", "OpenRouter devolveu um n\xFAmero de embeddings diferente do n\xFAmero de inputs.", {
          provider: "openrouter",
          endpoint,
          status: response.status,
          apiMessage: extractSafeApiMessage3(data),
          requestCount: 1
        });
      }
      const embeddings = new Array(inputs.length);
      const seen = /* @__PURE__ */ new Set();
      for (const item2 of data.data) {
        const itemIndex = typeof item2.index === "number" && Number.isInteger(item2.index) ? item2.index : null;
        if (itemIndex === null || itemIndex < 0 || itemIndex >= inputs.length || seen.has(itemIndex)) {
          return operationError("invalid-response", "OpenRouter devolveu \xEDndices de embeddings amb\xEDguos ou inv\xE1lidos.", {
            provider: "openrouter",
            endpoint,
            status: response.status,
            requestCount: 1
        ...
        if (!isValidEmbeddingVector(item2.embedding)) {
          return operationError("invalid-vector", "OpenRouter devolveu um embedding com valores inv\xE1lidos.", {
            provider: "openrouter",
            endpoint,
            status: response.status,
            apiMessage: extractSafeApiMessage3(data),
      ...
      if (seen.size !== inputs.length || embeddings.some((embedding) => !embedding)) {
        return operationError("invalid-response", "A resposta OpenRouter n\xE3o permite associar todos os embeddings aos inputs.", {
          provider: "openrouter",
          endpoint,
          status: response.status,
          requestCount: 1
      ...
      const dimension = embeddings[0].length;
      if (embeddings.some((embedding) => embedding.length !== dimension)) {
        return operationError("dimension-mismatch", "Os embeddings OpenRouter n\xE3o t\xEAm uma dimens\xE3o consistente.", {
          provider: "openrouter",
          endpoint,
          status: response.status,
          requestCount: 1
    ...
    return operationError(
      message.toLowerCase().includes("json") ? "invalid-response" : "connection",
      message.toLowerCase().includes("json") ? "Resposta JSON inv\xE1lida devolvida pelo OpenRouter." : "N\xE3o foi poss\xEDvel ligar ao OpenRouter para gerar embeddings.",
      {
        provider: "openrouter",
        endpoint,
        apiMessage: extractSafeApiMessage3(message),
        requestCount: 1
      }
    );
  } finally {
...
function createDeclarativeSettingsButtonRenderer(name, action, options = {}) {
  return (setting, group2) => {
    ...
    setting.addButton((button) => {
      var _a;
      button.setButtonText((_a = options.buttonText) != null ? _a : name);
...
function createSupportActionRenderer(name, description, buttonText, url, opener = openExternalUrl) {
  return createDeclarativeSettingsButtonRenderer(name, {
    run: () => opener(url),
    isDisabled: () => false
  }, { description, buttonText });
}
function createSupportEmailRenderer(strings, opener = openExternalUrl, clipboardWriter = writeToClipboard, notice = showNotice) {
  return (setting, _group) => {
    setting.setName(strings.settingsSupportEmail).setDesc(strings.settingsSupportEmailDescription);
    setting.descEl.createDiv({ text: SUPPORT_EMAIL_ADDRESS });
    setting.addButton((button) => button.setButtonText(strings.settingsSupportEmailCopyButton).setDisabled(false).onClick(() => {
      void clipboardWriter(SUPPORT_EMAIL_ADDRESS).then(() => {
        notice(strings.settingsSupportEmailCopySuccess);
      }, () => {
      });
    }));
    setting.addButton((button) => button.setButtonText(strings.settingsSupportEmailButton).setDisabled(false).onClick(() => opener(SUPPORT_EMAIL_URL)));
  };
}
...
function cloneWithLocalValue(snapshot, deviceId, key, value) {
  ...
  } else if ((existingDevice == null ? void 0 : existingDevice[key]) === value) {
    return snapshot;
  }
...
function hasSameEffectiveProviderTuple(snapshot, deviceId, domain, provider, model, baseUrl) {
  var _a;
  const device = (_a = snapshot.settings.deviceSettingsById) == null ? void 0 : _a[deviceId];
  const providerKey = domain === "analysis" ? "analysisProvider" : "embeddingsProvider";
  const modelKey = domain === "analysis" ? "analysisModel" : "embeddingsModel";
    ...
    async setLocalProviderValues(domain, provider, model, baseUrl, requestedEffects) {
      ...
      return withSerializedWrite(async () => {
        ...
        return persistAndRunEffects(
          previous,
          next,
          domain === "embedding" ? effectsForEmbeddingIdentityChange(previous, next, deviceId, effects) : effects
        );
      });
    }
  };
...
function createDeclarativeSettingsCandidateComposition(options) {
  ...
  const baseUrlControlDefinitions = /* @__PURE__ */ new Map();
...
var LinaSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  confirmDestructive(label, message) {
    return new Promise((resolve) => {
      ...
      const confirmation = new import_obsidian5.ConfirmationModal(this.app);
...
function getOwnershipPath() {
  return (0, import_obsidian7.normalizePath)(OWNERSHIP_FILE_PATH);
}
...
async function ensureOwnershipDirectory(adapter) {
  const dirPath = (0, import_obsidian7.normalizePath)(".lina");
  try {
    if (adapter.stat) {
      const stat = await adapter.stat(dirPath);
      if (!stat) {
...
async function readNotesIndexFile(app) {
  const adapter = app.vault.adapter;
  const notesPath = (0, import_obsidian8.normalizePath)(NOTES_INDEX_PATH);
  try {
...
async function readChunksIndexFile(app, strict) {
  const chunksPath = (0, import_obsidian8.normalizePath)(CHUNKS_INDEX_PATH);
  try {
    const adapter = app.vault.adapter;
...
async function saveTextIndex(app, indexedNotes, chunks, chunkingOptions, excludedNotes, exclusionsInfo, provenance) {
  ...
  try {
    ...
    const manifestPath = (0, import_obsidian8.normalizePath)(`${indexFolderPath}/manifest.json`);
    ...
    const files = [
      { path: (0, import_obsidian8.normalizePath)(`${indexFolderPath}/notes.json`), content: JSON.stringify(indexedNotes, null, 2) },
      { path: (0, import_obsidian8.normalizePath)(`${indexFolderPath}/${CHUNKS_FILE}`), content: chunks.map((item2) => JSON.stringify(item2)).join("\n") },
...
async function readTextIndexStatus(app, options = {}) {
  var _a;
  try {
    const manifestPath = (0, import_obsidian8.normalizePath)(MANIFEST_INDEX_PATH);
    const adapter = app.vault.adapter;
...
function normalizeAutomaticUpdatePath(path) {
  ...
  return (0, import_obsidian9.normalizePath)(trimmed).replace(/^\/+/, "");
}
...
function chunkText(filePath, content, options) {
  ...
  const chunks = [];
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const normalizedPath = (0, import_obsidian10.normalizePath)(filePath);
...
var IndexStatusModal = class extends import_obsidian11.Modal {
...
function getChunkId(value) {
  return isRecord7(value) && typeof value.chunkId === "string" && value.chunkId.length > 0 ? value.chunkId : void 0;
}
function isNonEmptyString(value) {
  return typeof value === "string" && value.trim().length > 0;
}
function isCandidateRecord(value) {
  return isRecord7(value) && typeof value.chunkId === "string";
}
...
var EMBEDDING_PERSISTENCE_FILES = Object.freeze({
  ...
  checkpoint: (0, import_obsidian12.normalizePath)(".lina/index/embeddings.checkpoint.jsonl"),
  checkpointMetadata: (0, import_obsidian12.normalizePath)(".lina/index/embeddings.checkpoint.meta.json"),
  checkpointTemporary: (0, import_obsidian12.normalizePath)(".lina/index/embeddings.checkpoint.tmp"),
  checkpointMetadataTemporary: (0, import_obsidian12.normalizePath)(".lina/index/embeddings.checkpoint.meta.tmp"),
  checkpointBackup: (0, import_obsidian12.normalizePath)(".lina/index/embeddings.checkpoint.backup"),
  checkpointMetadataBackup: (0, import_obsidian12.normalizePath)(".lina/index/embeddings.checkpoint.meta.backup"),
  embeddingsPublishTemporary: (0, import_obsidian12.normalizePath)(".lina/index/embeddings.publish.tmp"),
  embeddingsPublishBackup: (0, import_obsidian12.normalizePath)(".lina/index/embeddings.publish.backup"),
  manifestPublishTemporary: (0, import_obsidian12.normalizePath)(".lina/index/manifest.publish.tmp"),
  manifestPublishBackup: (0, import_obsidian12.normalizePath)(".lina/index/manifest.publish.backup")
});
...
async function renameEmbeddingPersistenceArtifact(adapter, oldPath, newPath, options = {}) {
  var _a;
  const sleep = (_a = options.sleep) != null ? _a : waitForRenameRetry;
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      if (!isTransientWindowsRenameError(error) || delayMs === void 0) throw error;
      await sleep(delayMs);
    }
  }
}
...
async function restoreCanonicalBackups(app) {
  ...
  if (!await fileExists(app, files.embeddingsPublishBackup) && await fileExists(app, files.manifestPublishBackup)) {
    try {
      ...
      if (isRecord8(manifestBackup) && manifestBackup.indexType === "text") {
        await removeIfExists(app, files.canonicalEmbeddings);
        await removeIfExists(app, files.canonicalManifest);
        await renameEmbeddingPersistenceArtifact(app.vault.adapter, files.manifestPublishBackup, files.canonicalManifest);
        return true;
...
function getDeviceCapabilities() {
  return resolveDeviceCapabilities(import_obsidian13.Platform);
}
...
function collectRecordsInChunkOrder(chunks, records, chunkIds, excludedChunkIds = /* @__PURE__ */ new Set()) {
  ...
  for (const record of records) {
    if (!isRecord9(record) || typeof record.chunkId !== "string") continue;
...
function calculateEmbeddingUpdatePlan(input) {
  ...
  const canonicalReadability = (_b = input.canonicalReadability) != null ? _b : !canonicalExists ? "missing" : input.canonicalRecords.length === 0 ? "empty" : "readable";
  ...
  const checkpointState = calculateEmbeddingState({
    chunks: input.chunks,
    canonicalRecords: (_c = input.checkpointRecords) != null ? _c : [],
    publishedIdentity: {},
    nextGenerationIdentity: input.targetIdentity,
    buildInput: input.buildInput,
    hashInput: input.hashInput
  ...
  const recoverableCheckpointRecords = collectRecordsInChunkOrder(
    input.chunks,
    (_d = input.checkpointRecords) != null ? _d : [],
  ...
  const chunksToGenerate = mode === "indeterminate" ? [] : input.chunks.filter((chunk) => !coveredChunkIds.has(chunk.chunkId));
  ...
  if (mode !== "indeterminate" && chunksToGenerate.length === 0) addReason2(reasons, "no-generation-needed");
...
function validateEmbeddingGenerationConfig(options) {
  ...
  if ((metadata == null ? void 0 : metadata.requiresApiKey) && !((_a = options.apiKey) == null ? void 0 : _a.trim())) {
    return operationError("configuration", `Chave API da ${metadata.label} em falta. Define uma chave local nas defini\xE7\xF5es do Lina.`, {
      provider,
      requestCount: 0
    });
  }
...
async function generateEmbeddingsForChunks(app, chunks, options) {
  ...
  for (let offset = 0; offset < totalToGenerate; offset += effectiveBatchSize) {
    ...
    if (checkpointWriteError !== null) {
      ...
      const checkpointError = operationError("persistence", `N\xE3o foi poss\xEDvel guardar o checkpoint de embeddings: ${String(checkpointWriteError)}`, {
        provider,
        requestCount: totalRequestCount
      });
      return {
...
async function readRuntimeEmbeddingSourceIdentityResult(app) {
  const adapter = app.vault.adapter;
  const manifestPath = (0, import_obsidian15.normalizePath)(".lina/index/manifest.json");
  const embeddingsPath = (0, import_obsidian15.normalizePath)(".lina/index/embeddings.jsonl");
  let info;
  try {
...
function buildRuntimeIndex(records, chunks, sourceIdentity) {
  const state = calculateEmbeddingState({
    chunks,
...
var RuntimeEmbeddingIndexCache = class {
  ...
  async load(source, chunks, revision) {
    ...
    try {
      ...
      this.actualReadRevision = revision;
      const content = await this.app.vault.adapter.read((0, import_obsidian15.normalizePath)(".lina/index/embeddings.jsonl"));
...
var BinaryEmbeddingCopyController = class {
  ...
  async readCanonicalManifest() {
    ...
    if (!isRecord10(value) || !isRecord10(value.embeddings) || !isRecord10(value.embeddingInput)) throw new Error("invalid");
    const embeddings = value.embeddings;
...
// src/search/textSearchModal.ts
var TextSearchModal = class extends import_obsidian16.Modal {
  ...
  doSearch() {
    ...
    const results = searchTextIndex(this.notes, this.chunks, query, {
      maxResults: 30,
      maxChunksPerNote: 3
    }).filter((result) => this.app.vault.getAbstractFileByPath(result.path) instanceof import_obsidian16.TFile);
    if (results.length === 0) {
  ...
  openNote(path) {
    ...
    if (!(file instanceof import_obsidian16.TFile)) {
      new import_obsidian16.Notice("Nota nao encontrada no vault.");
      return;
    }
...
var SemanticSearchModal = class extends import_obsidian17.Modal {
  ...
  async doSearch() {
    ...
    const safeChunks = chunks == null ? void 0 : chunks.filter((chunk) => {
      var _a2;
      if (!(this.app.vault.getAbstractFileByPath(chunk.path) instanceof import_obsidian17.TFile)) {
        return false;
      }
      if ((_a2 = this.plugin) == null ? void 0 : _a2.isIndexPathExcludedByUserRules(chunk.path)) {
        return false;
  ...
  openNote(path) {
    ...
    if (!(file instanceof import_obsidian17.TFile)) {
      new import_obsidian17.Notice(this.L.errorNoteNotFound);
      return;
    }
    void this.app.workspace.getLeaf().openFile(file);
    this.close();
...
var IndexDiagnosticModal = class _IndexDiagnosticModal extends import_obsidian18.Modal {
  constructor(app, plugin) {
    super(app);
    this.plugin = plugin;
  }
...
var DeviceDiagnosticsModal = class extends import_obsidian19.Modal {
  ...
  renderArtifactCard(container, title, artifact, details) {
    const card = container.createDiv({
      attr: {
        style: "border: 1px solid var(--background-modifier-border); border-radius: 6px; padding: 10px 14px; background: var(--background-secondary);"
      }
    });
    const header = card.createDiv({
...
function getResultKey(path, chunkId, origin) {
  var _a;
  return `${(0, import_obsidian20.normalizePath)(path)}::${(_a = chunkId != null ? chunkId : origin) != null ? _a : "note"}`;
}
async function loadEmbeddings2(app) {
  try {
    const adapter = app.vault.adapter;
    const path = (0, import_obsidian20.normalizePath)(".lina/index/embeddings.jsonl");
    const stat = await adapter.stat(path);
    if (!stat || stat.type !== "file") {
...
function combineResults(textResults, semanticResults, weights) {
  ...
  for (const textResult of textResults) {
    const key = (0, import_obsidian20.normalizePath)(textResult.path);
  ...
  for (const semanticResult of semanticResults) {
    const key = (0, import_obsidian20.normalizePath)(semanticResult.path);
    const existing = byNote.get(key);
    if (!existing) {
...
async function runHybridSearch(app, notes, chunks, query, config) {
  ...
  if (config.getRuntimeEmbeddingIndex) {
    ...
    const semanticResults2 = searchRuntimeSemanticIndex(queryResult2.embedding, runtimeIndex, chunks, {
      maxResults: 30,
...
function getPathInSameFolder(file, fileName) {
  ...
  if (separatorIndex < 0) return (0, import_obsidian21.normalizePath)(fileName);
  const folder = file.path.substring(0, separatorIndex);
  return (0, import_obsidian21.normalizePath)(`${folder}/${fileName}`);
}
...
function getPathInFolder(folderPath, fileName) {
  return (0, import_obsidian21.normalizePath)(folderPath ? `${folderPath}/${fileName}` : fileName);
}
function normalizePathForComparison(path) {
  return (0, import_obsidian21.normalizePath)(path).replace(/\\/g, "/").trim().toLowerCase();
}
function normalizeSuggestedFolderPath(suggestedFolder) {
  ...
  const normalized = (0, import_obsidian21.normalizePath)(parts.join("/")).replace(/^\/+/, "");
...
var _LinaSearchView = class _LinaSearchView extends import_obsidian21.ItemView {
  ...
  renderAskContextBlockedByUserExclusions() {
    ...
    new import_obsidian21.Notice(this.L.askExcludedByUserRules);
  }
  getNormalizedContextPath(file) {
    return (0, import_obsidian21.normalizePath)(file.path);
  }
  captureContextSelectionBeforeSidebarFocus() {
    var _a;
    const activeFile = this.app.workspace.getActiveFile();
    if (!(activeFile instanceof import_obsidian21.TFile) || activeFile.extension !== "md") {
      return;
    }
  ...
  clearContextSelectionIfDifferent(file) {
    ...
    if (!(file instanceof import_obsidian21.TFile) || this.lastContextSelection.path !== this.getNormalizedContextPath(file)) {
      this.lastContextSelection = void 0;
    }
  }
  filterChunksByUserContentRules(chunks) {
    ...
    return chunks.filter((chunk) => {
      if (!(this.app.vault.getAbstractFileByPath(chunk.path) instanceof import_obsidian21.TFile)) {
        return false;
      }
  ...
  filterNotesByFilteredChunks(notes, chunks, allChunks = chunks) {
    ...
    return notes.filter((note) => {
      if (!(this.app.vault.getAbstractFileByPath(note.path) instanceof import_obsidian21.TFile)) {
        return false;
      }
  ...
  getExistingVaultFolders() {
    return this.app.vault.getAllLoadedFiles().filter((file) => file instanceof import_obsidian21.TFolder).map((folder) => (0, import_obsidian21.normalizePath)(folder.path).replace(/^\/+|\/+$/g, "")).filter((path) => path.length > 0).sort((a, b) => a.localeCompare(b));
  }
  ...
  normalizeFolderPathForAnalysis(folderPath) {
    ...
    if (!trimmed) return "";
    return (0, import_obsidian21.normalizePath)(trimmed).replace(/^\/+|\/+$/g, "");
  }
  getFolderMarkdownNotes(folderPath, options) {
    ...
    const visitFolder = (currentFolder) => {
      for (const child of currentFolder.children) {
        ...
        } else if (includeSubfolders && child instanceof import_obsidian21.TFolder) {
          visitFolder(child);
        }
      }
    };
  ...
  getPreferredFolderAnalysisPath(folderChoices) {
    var _a, _b, _c, _d, _e;
    const activeFile = this.app.workspace.getActiveFile();
    const activeFolder = activeFile instanceof import_obsidian21.TFile ? getFolderPathForFile(activeFile) : "";
    const candidates = [
      activeFolder,
  ...
  isInboxFolderPath(folderPath) {
    var _a;
    const inboxPath = (0, import_obsidian21.normalizePath)(((_a = this.plugin.settings.inboxFolderPath) != null ? _a : "").trim()).replace(/^\/+|\/+$/g, "");
  ...
  applyEmbeddingWorkStatus(_state) {
    if (!this.viewOpen) return;
  ...
  async copySuggestedMetadataToClipboard(text) {
    try {
      ...
      new import_obsidian21.Notice(this.L.analysisCopyMetadataSuccess);
    } catch (error) {
      console.warn("Lina: n\xE3o foi poss\xEDvel copiar metadados sugeridos:", error);
      new import_obsidian21.Notice(this.L.analysisCopyError);
    }
  }
  renderPreservedSuggestedMetadata(container) {
    ...
    applyButton.addEventListener("click", () => {
      void this.applyPreservedMetadataToActiveNote().catch((error) => {
        const message = error instanceof Error ? error.message : String(error);
        new import_obsidian21.Notice(`${this.L.applySuggestionsErrorPrefix}: ${message}`);
      });
    });
  ...
  async refreshState(options = {}) {
    ...
    const isStale = () => !this.viewOpen || generation !== this.viewGeneration || refreshGeneration !== this.stateRefreshGeneration;
  ...
  confirmEmbeddingFullRebuild() {
    return new Promise((resolve) => {
      ...
      const originalOnClose = () => {
        import_obsidian21.Modal.prototype.onClose.call(modal);
      };
      modal.onClose = () => {
        originalOnClose();
        if (!settled) {
  ...
  async runSearch() {
    ...
    if (!selectedMode) {
      new import_obsidian21.Notice(this.L.searchSelectMode);
  ...
  async runAskCommand(userPrompt) {
    var _a;
    if (!userPrompt) {
      new import_obsidian21.Notice(this.L.askEmptyPrompt);
      this.setStatus(this.L.askEmptyPrompt);
      return;
    }
    const activeFile = this.app.workspace.getActiveFile();
    if (!(activeFile instanceof import_obsidian21.TFile)) {
      new import_obsidian21.Notice(this.L.askNoActiveNote);
      this.setStatus(this.L.askNoActiveNote);
      return;
    }
    if (activeFile.extension !== "md") {
      new import_obsidian21.Notice(this.L.analysisNonMarkdown);
  ...
  getSelectionRangeFromActiveMarkdownEditor(activeFile) {
    var _a, _b;
    const activeMarkdownView = this.app.workspace.getActiveViewOfType(import_obsidian21.MarkdownView);
    ...
    for (const leaf of this.app.workspace.getLeavesOfType("markdown")) {
      ...
      if (view instanceof import_obsidian21.MarkdownView && ((_b = view.file) == null ? void 0 : _b.path) === activeFile.path) {
  ...
  async runSemanticSearchGrouped(query, chunks) {
    ...
    if (runtimeIndex.sourceIdentity.inputVersion !== nextIdentity.inputVersion || runtimeIndex.sourceIdentity.prefixMode !== nextIdentity.prefixMode) {
  ...
  renderGroupedCards(cards, searchMode) {
    if (!this.viewOpen) return;
    const resolvableCards = cards.filter((card) => this.app.vault.getAbstractFileByPath(card.path) instanceof import_obsidian21.TFile);
  ...
  async handleEmbeddingGeneration() {
    ...
    if (request.status === "not-capable") {
      ...
      new import_obsidian21.Notice("Esta opera\xE7\xE3o requer um dispositivo produtor do Lina.");
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        new import_obsidian21.Notice(result.message || this.L.toastEmbeddingsSuccess);
      } else if (result.cancelled) {
        this.applyEmbeddingOperationState(completion.state);
        new import_obsidian21.Notice(result.message || this.L.statusEmbeddingGenerationCancelled);
      } else {
        const errorMsg = result.message || this.L.statusEmbeddingsError;
        this.setStatus(errorMsg);
        new import_obsidian21.Notice(errorMsg);
      }
      await this.refreshState();
      ...
      new import_obsidian21.Notice(`${this.L.statusEmbeddingsErrorPrefix}: ${msg}`);
    }
  }
  /**
...
<<<FIM_NOTA>>>`;
  ...
  async renderTagsCommandSuggestions(container, suggestedTags, applyTarget) {
    ...
    if (targetFile instanceof import_obsidian21.TFile) {
  ...
  async applySelectedTagsFromCommand(applyTarget) {
    ...
    if (normalizePathForComparison(activeFile.path) !== normalizePathForComparison(applyTarget.path)) {
      new import_obsidian21.Notice(this.L.tagsWrongNote);
      return;
    }
    const targetFile = this.app.vault.getAbstractFileByPath(applyTarget.path);
    if (!(targetFile instanceof import_obsidian21.TFile)) {
      new import_obsidian21.Notice(this.L.errorTargetNoteGone);
      return;
    }
    if (targetFile.extension !== "md") {
      new import_obsidian21.Notice(this.L.errorTargetNotMarkdown);
      return;
    }
    const selectedTags = this.getSelectedTagsFromStructuredSelections();
    if (selectedTags.length === 0) {
      new import_obsidian21.Notice(this.L.noItemSelected);
      return;
    }
    try {
      const originalContent = await this.app.vault.read(targetFile);
      if (this.contentMatchesUserExclusion(originalContent)) {
        new import_obsidian21.Notice(this.L.askApplyNoteExcluded);
        return;
      }
      ...
      if (newSelectedTags.length === 0) {
        new import_obsidian21.Notice(this.L.tagsNoChanges);
        return;
      }
      const confirmed = await this.confirmApplyTagsCommand(targetFile, newSelectedTags);
      if (!confirmed) {
        new import_obsidian21.Notice(this.L.operationCancelledNoChange);
        return;
      }
      ...
      new import_obsidian21.Notice(this.L.tagsApplySuccess);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian21.Notice(`${this.L.applySuggestionsErrorPrefix}: ${message}`);
    }
  ...
  async applySelectedYamlFromCommand(applyTarget, suggestedYaml) {
    ...
    if (normalizePathForComparison(activeFile.path) !== normalizePathForComparison(applyTarget.path)) {
      new import_obsidian21.Notice(this.L.yamlWrongNote);
      return;
    }
    const targetFile = this.app.vault.getAbstractFileByPath(applyTarget.path);
    if (!(targetFile instanceof import_obsidian21.TFile)) {
    ...
    try {
      ...
      if (newSelectedYamlKeys.length === 0) {
        new import_obsidian21.Notice(this.L.yamlNoChanges);
        return;
      }
      ...
      await this.app.vault.modify(targetFile, updatedContent);
      new import_obsidian21.Notice(this.L.yamlApplySuccess);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian21.Notice(`${this.L.applySuggestionsErrorPrefix}: ${message}`);
    }
  ...
  async applyAskResponseToNote(responseText, applyTarget, mode) {
    const textToApply = responseText.trim();
    if (!textToApply) {
      new import_obsidian21.Notice(this.L.analysisEmptyResponse);
      return;
    }
    const activeFile = this.app.workspace.getActiveFile();
    if (!(activeFile instanceof import_obsidian21.TFile)) {
    ...
    if ((0, import_obsidian21.normalizePath)(activeFile.path) !== (0, import_obsidian21.normalizePath)(applyTarget.path)) {
      new import_obsidian21.Notice(this.L.askApplyWrongNote);
      return;
    }
    const targetFile = this.app.vault.getAbstractFileByPath(applyTarget.path);
    if (!(targetFile instanceof import_obsidian21.TFile)) {
    ...
    try {
      ...
      if ((mode === "insert-below-selection" || mode === "replace-selection") && !selectionRange) {
        new import_obsidian21.Notice(this.L.askApplySelectionUnavailable);
        return;
      }
      let nextContent;
      if (mode === "append-to-note") {
        ...
        if (selectedContent !== selectionRange.selectedText) {
          new import_obsidian21.Notice(this.L.askApplySelectionChanged);
          return;
        }
      ...
      if (nextContent === originalContent) {
        new import_obsidian21.Notice(this.L.noAnalysisToApply);
        return;
      }
      ...
      new import_obsidian21.Notice(this.L.askApplySuccess);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian21.Notice(`${this.L.askApplyErrorPrefix}: ${message}`);
    }
  }
  ...
  async copyAiResponseToClipboard(responseText) {
    try {
      ...
      new import_obsidian21.Notice(this.L.analysisCopySuccess);
    } catch (error) {
  ...
  async applyPreservedMetadataToActiveNote() {
    ...
    if (!(targetFile instanceof import_obsidian21.TFile)) {
      new import_obsidian21.Notice(this.L.analysisNoFile);
      return;
    }
    if (targetFile.extension !== "md") {
      new import_obsidian21.Notice(this.L.errorTargetNotMarkdown);
    ...
    if (this.lastSuggestedMetadataScope === "batch" && (!this.lastSuggestedMetadataSourcePath || normalizePathForComparison(targetFile.path) !== normalizePathForComparison(this.lastSuggestedMetadataSourcePath))) {
      new import_obsidian21.Notice(this.L.analysisBatchMetadataWrongNote);
      return;
    }
    ...
    if (updatedContent === originalContent) {
      new import_obsidian21.Notice(this.L.analysisNoPreservedMetadataChanges);
      return;
    }
    await this.app.vault.modify(targetFile, updatedContent);
    new import_obsidian21.Notice(this.L.analysisPreservedMetadataApplied);
  }
  /**
  ...
  async applySelectedChanges() {
    ...
    if (renameFileSelected) {
      if (!result.suggestedTitle || result.suggestedTitle.trim().length === 0) {
        new import_obsidian21.Notice(this.L.titleEmptyNoRename);
        return;
      }
      if (!renameTargetName || !renameTargetPath) {
        new import_obsidian21.Notice(this.L.noSafeNameGenerated);
        return;
      }
      if (!moveFolderSelected && (0, import_obsidian21.normalizePath)(targetFile.path).toLowerCase() === (0, import_obsidian21.normalizePath)(renameTargetPath).toLowerCase()) {
        new import_obsidian21.Notice(this.L.suggestedNameSameAsCurrent);
        return;
      }
      if (!moveFolderSelected) {
        const existingTarget = this.app.vault.getAbstractFileByPath(renameTargetPath);
        if (existingTarget) {
          new import_obsidian21.Notice(this.L.fileAlreadyExistsDestNoRename);
          return;
        }
      }
    }
    if (moveFolderSelected) {
      ...
      if (!suggestedFolder.isValid) {
        new import_obsidian21.Notice(this.L.folderNotValid);
        return;
      }
      moveFolderPath = suggestedFolder.path;
      const destinationFolder = this.app.vault.getAbstractFileByPath(moveFolderPath);
      if (!(destinationFolder instanceof import_obsidian21.TFolder)) {
        new import_obsidian21.Notice(this.L.folderNotExists);
        new import_obsidian21.Notice(this.L.folderAutoCreateNotAllowed);
        return;
      }
      ...
      if (normalizePathForComparison(currentFolderForMove) === normalizePathForComparison(moveFolderPath != null ? moveFolderPath : "")) {
        new import_obsidian21.Notice(this.L.noteAlreadyInFolder);
        return;
      }
    }
    ...
    if (pathWillChange) {
      ...
      if (existingTarget) {
        if (moveFolderSelected) {
          new import_obsidian21.Notice(this.L.fileAlreadyExistsDestNoMove);
        } else {
          new import_obsidian21.Notice(this.L.fileAlreadyExistsDestNoRename);
        }
        return;
    ...
    try {
      ...
      if (pathWillChange) {
        ...
        if (existingTarget) {
          ...
          if (moveFolderSelected) {
            new import_obsidian21.Notice(this.L.noteMovedSuccess);
          } else if (renameFileSelected) {
            new import_obsidian21.Notice(this.L.fileRenamedSuccess);
          }
        }
      }
      if (content !== originalContent) {
        new import_obsidian21.Notice(this.L.suggestionsApplied);
      }
      if (selectedYamlKeys.length > 0) {
      }
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      new import_obsidian21.Notice(`${this.L.applySuggestionsErrorPrefix}: ${msg}`);
    }
  }
  /**
...
`;
  ...
  async analyzeMarkdownFile(file, options) {
    ...
    if (!(currentFile instanceof import_obsidian21.TFile)) {
      this.analysisResultEl.createDiv({
  ...
  async analyzeInboxNotes() {
    ...
    const inboxFolderPath = (0, import_obsidian21.normalizePath)(((_a = this.plugin.settings.inboxFolderPath) != null ? _a : "").trim());
    if (!inboxFolderPath) {
      this.analysisResultEl.createDiv({
...
<<<FIM_NOTA>>>`;
  ...
  async moveInboxAnalysisFile(file, suggestedFolder, statusEls, moveButton, pathEl) {
    const currentFile = this.app.vault.getAbstractFileByPath(file.path);
    if (!(currentFile instanceof import_obsidian21.TFile)) {
      new import_obsidian21.Notice(this.L.errorNoteNoLongerExists);
      return;
    }
    if (currentFile.extension !== "md") {
      new import_obsidian21.Notice(this.L.errorFileNotMarkdown);
      return;
    }
    ...
    if (!resolution.canMove || !resolution.finalTargetPath || !resolution.resolvedFolderPath) {
      new import_obsidian21.Notice(resolution.reason);
    ...
    if (!confirmed) {
      new import_obsidian21.Notice(this.L.operationCancelledNoMove);
      return;
    }
    const latestFile = this.app.vault.getAbstractFileByPath(currentFile.path);
    if (!(latestFile instanceof import_obsidian21.TFile)) {
      new import_obsidian21.Notice(this.L.errorNoteNoLongerExists);
      return;
    }
    ...
    if (!finalResolution.canMove || !finalResolution.finalTargetPath || !finalResolution.resolvedFolderPath) {
      new import_obsidian21.Notice(finalResolution.reason);
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian21.Notice(`${this.L.errorMoveNotePrefix}: ${message}`);
    }
  }
  async openInboxAnalysisFile(file) {
    const currentFile = this.app.vault.getAbstractFileByPath(file.path);
    if (!(currentFile instanceof import_obsidian21.TFile)) {
      new import_obsidian21.Notice(this.L.errorNoteSelectedGone);
      return false;
    }
    if (currentFile.extension !== "md") {
      new import_obsidian21.Notice(this.L.errorFileNotMarkdown);
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian21.Notice(`${this.L.errorOpenNotePrefix}: ${message}`);
      return false;
    }
  }
  ...
  openNote(path) {
    const file = this.app.vault.getAbstractFileByPath(path);
    if (!(file instanceof import_obsidian21.TFile)) {
      new import_obsidian21.Notice(this.L.errorNoteNotFound);
      return;
    }
    void this.app.workspace.getLeaf().openFile(file);
  }
...
var OwnershipGate = class {
  ...
  async validateArtifactAsync(provenance) {
...
var LinaPlugin = class extends import_obsidian22.Plugin {
  constructor() {
    super(...arguments);
    this.indexedNotes = [];
    this.indexedChunks = [];
  ...
  async onload() {
    ...
    this.addRibbonIcon("search", this.L.mainRibbonOpenLina, () => {
      void this.activateLinaSearchView().catch((error) => {
        ...
        const message = error instanceof Error ? error.message : String(error);
        new import_obsidian22.Notice(`${this.L.mainNoticeOpenLinaErrorPrefix}. ${message}`);
      });
    });
    new import_obsidian22.Notice(this.L.mainNoticeLinaLoaded);
    this.addCommand({
      id: "pesquisar",
      ...
      callback: () => {
        void (async () => {
          try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            new import_obsidian22.Notice(`${this.L.mainNoticeOpenSideSearchErrorPrefix}. ${message}`);
          }
        })();
      }
    });
    this.addCommand({
      ...
      callback: () => {
        void (async () => {
          try {
            new import_obsidian22.Notice(this.L.mainNoticeRebuildingTextIndex);
            const result = await this.rebuildTextIndex();
            new import_obsidian22.Notice(result.message);
          } catch (error) {
            console.error("Lina: failed to rebuild text index", error);
            const message = error instanceof Error ? error.message : String(error);
            new import_obsidian22.Notice(`${this.L.mainNoticeRebuildTextIndexErrorPrefix}. ${message}`);
          }
        })();
      }
    });
      ...
      callback: () => {
        void (async () => {
          try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            new import_obsidian22.Notice(`${this.L.mainNoticeReadTextIndexStateErrorPrefix}. ${message}`);
          }
        })();
      }
    });
        ...
        void (async () => {
          try {
            ...
            if (!loaded || this.indexedNotes.length === 0) {
              new import_obsidian22.Notice(this.L.mainNoticeTextIndexEmpty);
              return;
            }
            ...
            const message = error instanceof Error ? error.message : String(error);
            new import_obsidian22.Notice(`${this.L.mainNoticeSearchTextIndexErrorPrefix}. ${message}`);
          }
        })();
      }
    });
      ...
      callback: () => {
        void (async () => {
          try {
            ...
            if (request.status !== "accepted") {
              if (request.status === "already-running") {
                new import_obsidian22.Notice(this.L.toastEmbeddingsAlreadyRunning);
                return;
              }
              if (request.status === "text-index-busy") {
                new import_obsidian22.Notice(this.L.mainNoticeTextIndexBusyForEmbeddings);
                return;
              }
              if (request.status === "not-capable") {
                new import_obsidian22.Notice(PRODUCER_OPERATION_UNAVAILABLE_MESSAGE);
                return;
              }
              new import_obsidian22.Notice(this.L.toastEmbeddingsError);
              return;
            }
            new import_obsidian22.Notice(this.L.toastGeneratingEmbeddings);
            const completion = await request.completion;
            new import_obsidian22.Notice(completion.result.message);
          } catch (error) {
            console.error("Lina: failed to generate embeddings:", error);
            const msg = error instanceof Error ? error.message : String(error);
            new import_obsidian22.Notice(`${this.L.mainNoticeGenerateEmbeddingsErrorPrefix}. ${msg}`);
          }
        })();
      }
    });
    this.addCommand({
      ...
      callback: () => {
        ...
        if (result === "cancel-requested") {
          new import_obsidian22.Notice(this.L.toastEmbeddingGenerationCancelling);
          return;
        }
        if (result === "already-cancelling") {
          new import_obsidian22.Notice(this.L.toastEmbeddingGenerationAlreadyCancelling);
          return;
        }
        new import_obsidian22.Notice(this.L.toastNoActiveEmbeddingGeneration);
      }
    });
    this.addCommand({
        ...
        void (async () => {
          try {
            ...
            if (!status || !status.exists) {
              new import_obsidian22.Notice(this.L.mainNoticeNoLocalEmbeddings);
              return;
            }
            ...
            const msg = error instanceof Error ? error.message : String(error);
            new import_obsidian22.Notice(`${this.L.mainNoticeReadEmbeddingsStateErrorPrefix}. ${msg}`);
          }
        })();
      }
    });
      ...
      callback: () => {
        try {
          ...
          if (!embeddingConfig.baseUrl) {
            new import_obsidian22.Notice(this.L.mainNoticeOllamaUrlMissing);
            return;
          }
          ...
          const msg = error instanceof Error ? error.message : String(error);
          new import_obsidian22.Notice(`${this.L.mainNoticeOpenSemanticSearchErrorPrefix}. ${msg}`);
        }
      }
    });
    this.addCommand({
      ...
      callback: () => {
        try {
          ...
          const msg = error instanceof Error ? error.message : String(error);
          new import_obsidian22.Notice(`${this.L.mainNoticeOpenIndexDiagnosticErrorPrefix}. ${msg}`);
        }
      }
    });
    this.addCommand({
      ...
      callback: () => {
        void (async () => {
          try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            new import_obsidian22.Notice(`${this.L.mainNoticeOpenDeviceDiagnosticsErrorPrefix}. ${msg}`);
          }
        })();
      }
    });
    ...
    (_a = this.maintenanceEngine) != null ? _a : this.maintenanceEngine = new MaintenanceEngine({
      ...
      binaryWorker: new BinaryWorker({
        ...
        onAutomaticMaintenanceFailure: (summary) => {
          ...
          new import_obsidian22.Notice(this.L.settingsBinaryAutomaticWarning);
        }
      }),
  ...
  async logTextIndexStartupStatus() {
    ...
    try {
      ...
      new import_obsidian22.Notice(`${this.L.mainNoticeTextIndexLoadErrorPrefix}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  buildEmbeddingGenerationFailureMessage(config, result) {
    ...
    switch (category) {
      ...
      case "authorization":
        ...
        break;
      case "billing":
        hint = "Verifica os cr\xE9ditos ou a fatura\xE7\xE3o do provider.";
        break;
      case "rate-limit":
      ...
      case "unsupported-provider":
        ...
        break;
      case "persistence":
        hint = "A publica\xE7\xE3o local dos embeddings falhou. Verifica se o vault n\xE3o est\xE1 temporariamente bloqueado.";
        break;
      case "input-rejected":
    ...
    if (this.settings.updateIndexOnStartup && getDeviceCapabilities().canMaintainTextIndex) {
      if (textIndexStatus.isUsable) {
        if (textIndexStatus.usability === "stale") {
          new import_obsidian22.Notice("Lina: \xEDndice textual desatualizado.");
        }
        return;
      }
      ...
      if (!hadPreviousIndex) {
        await this.saveDataToDisk();
        new import_obsidian22.Notice(`Lina criou o \xEDndice com ${result.indexData.entries.length} notas.`);
        return;
      }
      if (hasChanges) {
        await this.saveDataToDisk();
        new import_obsidian22.Notice(`Lina atualizou o \xEDndice: ${result.addedCount} novas, ${result.updatedCount} alteradas, ${result.removedCount} removidas.`);
      }
      return;
    }
    if (!this.settings.checkSyncOnStartup) return;
    if (textIndexStatus.usability === "missing") {
      new import_obsidian22.Notice("Lina: \xEDndice ainda n\xE3o criado.");
      return;
    }
    if (textIndexStatus.usability === "invalid") {
      new import_obsidian22.Notice("Lina: \xEDndice textual indispon\xEDvel.");
      return;
    }
    if (textIndexStatus.usability === "stale") {
      new import_obsidian22.Notice("Lina: \xEDndice textual desatualizado.");
