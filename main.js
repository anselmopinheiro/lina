...
function isLegacyAutoProviderProfile(profile, settings) {
  ...
  return profile.name === defaultName && profile.baseUrl === defaults.baseUrl && profile.model === defaults.model && (profile.requestTimeoutSeconds ?? 60) === defaults.requestTimeoutSeconds && (profile.isLocal ?? false) === (defaults.isLocal ?? false);
}
...
var LinaSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsInboxFolder).setDesc(this.L.settingsInboxFolderDesc).addText(
      (text) => text.setPlaceholder("00_Inbox").setValue(this.plugin.settings.inboxFolderPath ?? "00_Inbox").onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsInboxMaxNotes).setDesc(this.L.settingsInboxMaxNotesDesc).addText(
      (text) => text.setPlaceholder("10").setValue(String(this.plugin.settings.maxInboxNotesToAnalyze ?? 10)).onChange(async (value) => {
        const num = parseInt(value, 10);
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsCheckSyncOnStartup).setDesc(this.L.settingsCheckSyncOnStartupDesc).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.checkSyncOnStartup ?? false).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsUpdateIndexOnStartup).setDesc(this.L.settingsUpdateIndexOnStartupDesc).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.updateIndexOnStartup ?? false).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsAutoUpdateIndex).setDesc(this.L.settingsAutoUpdateIndexDesc).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoUpdateIndexOnFileChanges ?? false).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsDebugIndex).setDesc(this.L.settingsDebugIndexDesc).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.debugIndexUpdates ?? false).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsExcludedFolders).setDesc(this.L.settingsExcludedFoldersDesc).addTextArea(
      (text) => text.setPlaceholder("03_Pessoal/").setValue(this.plugin.settings.indexExcludedFolders ?? "").onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsExcludedTerms).setDesc(this.L.settingsExcludedTermsDesc).addTextArea(
      (text) => text.setPlaceholder("senha\npassword\ntoken").setValue(this.plugin.settings.indexExcludedPathContains ?? "").onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsTextWeight).setDesc(this.L.settingsTextWeightDesc).addText(
      (text) => text.setPlaceholder("0.7").setValue(String(this.plugin.settings.hybridSearchTextWeight ?? 0.7)).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsSemanticWeight).setDesc(this.L.settingsSemanticWeightDesc).addText(
      (text) => text.setPlaceholder("0.3").setValue(String(this.plugin.settings.hybridSearchSemanticWeight ?? 0.3)).onChange(async (value) => {
        const num = Number.parseFloat(value);
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsYamlEnabled).setDesc(this.L.settingsYamlEnabledDesc).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.yamlSuggestionsEnabled ?? true).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsYamlProperties).setDesc(this.L.settingsYamlPropertiesDesc).addText(
      (text) => text.setPlaceholder("tipo, projeto, area, contexto, estado, tags").setValue(this.plugin.settings.yamlAllowedProperties ?? "tipo, projeto, area, contexto, estado, tags").onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsYamlIncludeTags).setDesc(this.L.settingsYamlIncludeTagsDesc).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.yamlIncludeTags ?? true).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.L.settingsMaxTags).setDesc(this.L.settingsMaxTagsDesc).addText(
      (text) => text.setPlaceholder("8").setValue(String(this.plugin.settings.maxSuggestedTags ?? 8)).onChange(async (value) => {
        const num = parseInt(value, 10);
    ...
    if (manifestStat && manifestStat.type === "file") {
      try {
        ...
        if (emb && manifest.embeddingsEnabled) {
          ...
          manifestModel = emb.model ?? "";
          manifestProvider = emb.provider ?? "";
          manifestDimensions = emb.dimensions ?? 0;
          manifestUpdatedAt = emb.updatedAt ?? "";
        }
        const embeddingInput = manifest.embeddingInput;
        if (embeddingInput) {
          manifestPrefixMode = embeddingInput.prefixMode ?? "none";
        }
      } catch {
      }
    }
...
function getResultKey(path, chunkId, origin) {
  return `${(0, import_obsidian11.normalizePath)(path)}::${chunkId ?? origin ?? "note"}`;
}
...
function combineResults(textResults, semanticResults, textWeight, semanticWeight) {
  ...
  for (const entry of byNote.values()) {
    ...
    const textScore = textResult ? normalisedTextScores.get(getResultKey(textResult.path, textResult.chunkId, textResult.origin)) ?? 0 : void 0;
  ...
  mergedResults.sort((a, b) => {
    ...
    if ((b.textScore ?? 0) !== (a.textScore ?? 0))
      return (b.textScore ?? 0) - (a.textScore ?? 0);
    if ((b.semanticSimilarity ?? 0) !== (a.semanticSimilarity ?? 0))
...
var _LinaSearchView = class extends import_obsidian12.ItemView {
  ...
  isInboxFolderPath(folderPath) {
    const inboxPath = (0, import_obsidian12.normalizePath)((this.plugin.settings.inboxFolderPath ?? "").trim()).replace(/^\/+|\/+$/g, "");
  ...
  resolveFolderMove(suggestedFolder, existingFolders, currentFolderPath, currentFileName, currentFilePath) {
    ...
    const baseResolution = (values) => {
      const resolvedFolderPath2 = values.resolvedFolderPath ?? null;
      const finalTargetPath2 = values.finalTargetPath ?? (currentFileName && resolvedFolderPath2 ? getPathInFolder(resolvedFolderPath2, currentFileName) : null);
      ...
      return {
        ...
        canMove,
        reason: values.reason ?? "A pasta sugerida n\xE3o \xE9 v\xE1lida."
      };
    };
    if (!normalized.isValid) {
  ...
  async refreshState() {
    ...
    const hasProviderMismatch = !!embeddingStatus?.exists && embeddingStatus?.provider && (getLocalEmbeddingsProvider() || this.plugin.settings.embeddingProvider || "ollama").toLowerCase() !== embeddingStatus.provider.toLowerCase();
    const hasModelMismatch = !!embeddingStatus?.exists && embeddingStatus?.model && (getLocalEmbeddingsModel() || this.plugin.settings.embeddingModel || "") !== embeddingStatus.model;
  ...
  getSelectedSearchMode() {
    if (this.searchModeRadioButtons.textual?.checked)
      return "textual";
    if (this.searchModeRadioButtons.hibrida?.checked)
      return "hibrida";
    if (this.searchModeRadioButtons.semantica?.checked)
      return "semantica";
    return null;
  }
  async runSearch() {
...
<<<FIM_NOTA>>>`;
  ...
  async renderStructuredPreview(result, relatedNotesCount, relatedNotes = [], targetFile) {
    ...
    const analysisFile = targetFile ?? this.app.workspace.getActiveFile() ?? void 0;
    ...
    if (rawSuggestedFolder.length > 0) {
      ...
      const destinationPath = folderResolution.finalTargetPath ?? void 0;
...
`;
  ...
  async analyzeInboxNotes() {
    ...
    this.analysisResultEl.empty();
    this.analysisResultEl.style.display = "block";
    const inboxFolderPath = (0, import_obsidian12.normalizePath)((this.plugin.settings.inboxFolderPath ?? "").trim());
    if (!inboxFolderPath) {
      this.analysisResultEl.createDiv({
    ...
    for (let index = 0; index < filesToAnalyze.length; index++) {
      ...
      try {
        ...
        if (!json || error) {
          results.push({ file, error: error ?? "Resposta JSON inv\xE1lida." });
          continue;
        }
...
<<<FIM_NOTA>>>`;
  ...
  renderHighlightedCard(card, searchMode) {
    ...
    if (isSemantic) {
      ...
      const snippetInfo = getSearchSnippetDisplay(card);
      const originLabel = snippetInfo?.isFallback ? snippetInfo.text : getReadableSearchOrigin(card.origin);
      const titleEl = cardEl.createEl("strong");
...
var LinaPlugin = class extends import_obsidian13.Plugin {
  ...
  async loadDataFromDisk() {
    ...
    this.indexData = data?.index ?? void 0;
  }
